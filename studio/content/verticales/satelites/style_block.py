# =====================================================================
# CO.DE Academy — "Satelites: la maquina que no se cae" (curso VERTICAL).
#
...
# `satelites.py` con numpy y semilla fija. Las CONSTANTES fisicas (mu, el
# radio terrestre, el dia sidereo) no son medidas de este render: cuando
# aparecen, van en gris, como cualquier dato externo.
# =====================================================================
import math
import os
import sys
...
C_MEDIDO = "#22d3ee"      # cian: TODA cifra calculada por la libreria aqui
C_SAT = "#f59e0b"         # ambar: el satelite, su orbita, su traza
C_TIERRA = "#34d399"      # verde: el planeta, el suelo, la estacion, tu
C_ENLACE = "#7c3aed"      # violeta: el haz, la huella, la cobertura, el ISL
C_PERDIDO = "#ea580c"     # naranja: el disparo que cae, el hueco, el corte
C_EJE = "#31414f"         # mobiliario: ejes, reticula, mapa base
C_EXTERNO = CODE_MUTED    # gris: constantes y datos que NO se miden aqui
C_TINTA = CODE_INK

# Alias del rol "instrumento" para las piezas heredadas del curso 26 (la
# barra de escaneo de la intro): aqui el instrumento es el propio satelite.
C_REGLA = C_SAT
...
def poli(pts, color=C_SAT, grosor=2.6, opacidad=1.0, suave=False):
...
def globo(radio=2.5, y=None, color=C_TIERRA, relleno=0.10):
    """La Tierra: un disco con su borde vivo. El centro va donde se pida.

    Es la pieza mas repetida del curso, asi que nace aqui y no en cada clip:
    si el planeta cambia de sitio entre piezas, el ojo lo nota en el corte.
    """
    c = Circle(radius=radio, stroke_color=color, stroke_width=3.0,
               fill_color=color, fill_opacity=relleno)
    c.move_to(UP * (Y_ESCENA if y is None else y))
    return c


def anillo_orbita(globo_mob, altitud_km, color=C_SAT, grosor=2.0,
                  opacidad=0.55, discontinua=True):
    """Orbita circular A ESCALA REAL sobre un `globo`: el radio sale de la
    altitud, no del gusto de nadie. A 400 km el anillo casi roza el planeta,
    y eso es justo lo que hay que enseñar."""
    r = globo_mob.width / 2 * (1.0 + altitud_km / sa.R_TIERRA_KM)
    c = Circle(radius=r, stroke_color=color, stroke_width=grosor,
               stroke_opacity=opacidad)
    c.move_to(globo_mob.get_center())
    return DashedVMobject(c, num_dashes=90) if discontinua else c
