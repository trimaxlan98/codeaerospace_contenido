# =====================================================================
# CO.DE Academy — "Funciones con nombre propio" (curso 34, VERTICAL,
# estilo LIENZO, NARRADO).
#
...
#
# El lenguaje visual vive en `lienzo.py` y la materia en `especiales.py`.
# Aqui queda lo que es de ESTE curso.
#
# QUE CAPA OCUPA (y por que importa al escribir una pieza):
#
#   Una funcion tiene nombre propio porque ninguna combinacion de las
#   elementales hacia su trabajo. Cada pieza presenta UNA, con el
#   problema que la obligo a existir y UNA cifra que solo ella sabe dar.
#
#   Lo que NO se cuenta aqui, porque ya tiene curso: el caos (12), la
#   autosemejanza y la dimension fractal (26), el cambio de dominio (32)
#   y los sistemas (33). La mariposa de Lorenz aparece en la pieza 14
#   como OBJETO —una curva que no se corta nunca— y no como teoria del
#   caos; si una pieza se pone a hablar de dimension o de Lyapunov, se ha
#   salido de su capa.
#
# EL RITMO ES EL DEL CURSO MUDO, Y ES A PROPOSITO:
#
#   Este curso SI lleva voz (`es-MX-JorgeNeural`) y cama de SFX. Aun asi
#   `leer()` mantiene el suelo de 1.8 s que heredamos de los cursos 32 y
#   33. Dos razones, las dos medidas:
#
...
#      AMBAR = la funcion con nombre propio, CIAN = aquello contra lo que
#      se la compara (la elemental, la aproximacion, el caso facil).
#   4. La cifra es tinta; la etiqueta dice de donde sale (ambar =
#      calculada aqui en el render, apagada = dada o elegida).
#
# Ninguna cifra en pantalla se inventa: todas salen de `especiales.py`
# durante el render, y los 155 invariantes de `sonda_especiales.py`
...
class Pieza(Scene):
    ...
    NOMBRE = None            # el nombre de la funcion, en la portada
