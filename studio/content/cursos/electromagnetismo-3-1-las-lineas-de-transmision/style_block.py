# =====================================================================
# CO.DE Academy - "Electromagnetismo · 3.1 Las líneas de transmisión".
...
# discrepar.
F_RED = 50.0                            # Hz, la frecuencia de la red
LAMBDA_RED_KM = lambda_de(F_RED) / 1000.0     # ~6000 km: una recta en la
                                               # escala de un cable
F_GIGA = 1.0e9                          # Hz, RF domestica (1 GHz)
LAMBDA_GIGA_M = lambda_de(F_GIGA)       # ~0.30 m: caben varias en el cable
Z0_RF = z0_coaxial(3.49)                # ~50 ohm: la moneda de la RF
Z0_TV = z0_coaxial(6.52)                # ~75 ohm: la moneda de la TV/SAT
# No hay una pieza de la libreria para la atenuacion vs frecuencia de un
# coaxial (falta en electromagnetismo.py, ver informe): se modela el
# efecto piel del RG-6 domestico con el mismo exponente 1/2 que usa la
# libreria en sus demas curvas fisicas (atenuacion ~ sqrt(f)), calibrado
# para que a 100 MHz de el valor publicado del RG-6 (6 dB/100 m).
COEF_ATEN_RG6 = 0.6                     # dB / 100 m / sqrt(MHz)


def atenuacion_rg6(f_mhz):
    """dB/100 m de un coaxial domestico tipo RG-6 (efecto piel ~ sqrt(f))."""
    return COEF_ATEN_RG6 * math.sqrt(float(f_mhz))


ATEN_100M = atenuacion_rg6(100.0)       # 6.0 dB/100 m a 100 MHz
ATEN_1G = atenuacion_rg6(1000.0)        # ~19.0 dB/100 m a 1 GHz
F_SAT_GHZ = 12.0                        # Ku descendente del satelite
ATEN_SAT = atenuacion_rg6(F_SAT_GHZ * 1000.0)  # dB/100 m a 12 GHz: enorme
