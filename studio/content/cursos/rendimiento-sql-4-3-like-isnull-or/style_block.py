# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 4.3 LIKE, ISNULL y OR".
...
M = S.MEDIDO
FILAS_CLI = M["filas_clientes"]                                 # 200,000
LIKE_PREFIJO = M["like_prefijo"]                                # 55 (motor)
LIKE_SUFIJO = M["like_sufijo"]                                  # 1,012 (motor)
GARCIA_TOTAL = S.filas_apellido("Garcia")                       # 10,165
ISNULL_MAL = M["isnull_mal"]                                    # 535 (motor)
ISNULL_BIEN = M["isnull_bien"]                                  # 3 (motor)
TEL_NULOS = S.telefonos_nulos()                                 # 39,854
EMAIL_VARCHAR = M["email_varchar"]                              # 3 (motor)
OR_DOS_SEEKS = M["or_dos_seeks"]                                # 14 (motor)
C501 = S.filas_cliente(501)                                     # 191
FECHA_FILAS = S.filas_rango("2026-09-19", "2030-01-01")         # 1,405 (>= fecha)
...
# Lista ILUSTRATIVA y ordenada de apellidos (apellidos reales de
# S.APELLIDOS, sin acentos): NO es la tabla de 200,000 clientes, es un
# dibujo del ORDEN de un indice. "Garcia" se repite para que el tramo del
# prefijo se vea como un tramo (varias filas seguidas), no como una fila
# suelta; es el UNICO apellido de la muestra que empieza por "Gar" o
# termina en "cia", asi que los dos LIKE marcan exactamente el mismo bloque.
COLUMNA_IZQ = ["Cruz", "Diaz", "Flores", "Garcia", "Garcia", "Garcia",
              "Garcia", "Garcia", "Gomez", "Gonzalez", "Hernandez",
              "Jimenez"]
COLUMNA_DER = ["Lopez", "Martinez", "Mendoza", "Morales", "Perez",
              "Ramirez", "Reyes", "Rodriguez", "Ruiz", "Sanchez", "Torres",
              "Vazquez"]
GARCIA_DESDE, GARCIA_HASTA = 3, 7        # indices (col izq) del tramo Garcia


def fila_apellido(nombre, ancho=1.85, alto=0.32, font_size=20):
    """Un renglon de la lista ordenada: el apellido en Space Mono sobre un
    fondo neutro (el color del fondo es lo que se enciende despues)."""
    fondo = Rectangle(width=ancho, height=alto, stroke_width=1.0,
                      stroke_color=C_TENUE, stroke_opacity=0.4,
                      fill_color=C_TENUE, fill_opacity=0.08)
    t = Text(nombre, font=FUENTE_HUD, font_size=font_size, color=C_TITULO)
    if t.width > ancho - 0.16:
        t.scale_to_fit_width(ancho - 0.16)
    t.move_to(fondo)
    g = VGroup(fondo, t)
    g.fondo = fondo
    return g
...
def columna_apellidos(nombres, ancho=1.85, alto=0.32, buff=0.045):
    col = VGroup(*[fila_apellido(n, ancho, alto) for n in nombres])
    col.arrange(DOWN, buff=buff)
    return col


def corchete(y_arriba, y_abajo, x, color, tick=0.14):
    """Dos marcas (una arriba, una abajo) que abrazan un tramo de la lista:
    el tronco vertical mas los dos remates horizontales."""
    tronco = Line([x, y_abajo, 0], [x, y_arriba, 0], stroke_color=color,
                 stroke_width=3)
    r_sup = Line([x, y_arriba, 0], [x + tick, y_arriba, 0], stroke_color=color,
                stroke_width=3)
    r_inf = Line([x, y_abajo, 0], [x + tick, y_abajo, 0], stroke_color=color,
                stroke_width=3)
