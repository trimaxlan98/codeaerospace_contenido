# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 7.1 La caja negra".
...
# La regresion de esta leccion ES el sesgo de parametros de 6.1: el mismo
# procedimiento del mayorista, dos planes guardados en momentos distintos.
M = S.MEDIDO
SNIFF_A = M["sniff_1_luego_501"][0]     # 22,363 - plan A (scan), motor
SNIFF_B = M["sniff_501_luego_1"][1]     # 459,555 - plan B (seek+lookup), motor
ERROR_ABORT = M["error_abort"]          # 8778 - motor
...
ALTO_MAX_BARRA = 2.3
_LOG_MAX = math.log10(SNIFF_B)


def alto_log(valor):
    """Altura (unidades) de una barra de intervalo en escala log10, con
    SNIFF_B (459,555) como techo: la MISMA escala en los clips 2 y 3."""
    return ALTO_MAX_BARRA * math.log10(max(valor, 1)) / _LOG_MAX


def barra_intervalo(alto, color, ancho=0.62):
    return Rectangle(width=ancho, height=max(alto, 0.05), stroke_width=0,
                     fill_color=color, fill_opacity=0.9)


def eje_intervalos(n, ancho=10.2, y=-1.6, color=None):
    """Linea de tiempo con `n` intervalos repartidos: devuelve el dibujo y
    las posiciones x (para plantar barras encima)."""
    color = C_TENUE if color is None else color
    xs = np.linspace(-ancho / 2 + 0.5, ancho / 2 - 0.5, n)
    linea = Line([-ancho / 2, y, 0], [ancho / 2, y, 0], stroke_color=color,
                stroke_width=2.2)
    marcas = VGroup(*[Line([x, y - 0.1, 0], [x, y + 0.1, 0], stroke_color=color,
                          stroke_width=2) for x in xs])
    eje = VGroup(linea, marcas)
    return eje, xs


def plan_a(pos=ORIGIN, color=None):
    """Plan A: un scan completo, un solo operador (el mismo de 6.1)."""
    color = C_TITULO if color is None else color
    op = S.operador("Scan pedidos", color=color, ancho=3.0, alto=0.85,
                    font_size=22)
    op.move_to(pos)
    return op


def plan_b(pos=ORIGIN, color=None, buff=1.7):
    """Plan B: Seek + Lookup, leido de derecha a izquierda."""
    color = C_TITULO if color is None else color
    lookup = S.operador("Lookup", color=color, ancho=2.1, alto=0.82,
                        font_size=20)
    seek = S.operador("Seek", color=color, ancho=1.8, alto=0.82, font_size=20)
    grupo = VGroup(lookup, seek)
    grupo.arrange(RIGHT, buff=buff)
    grupo.move_to(pos)
    flecha = Arrow(seek.get_left(), lookup.get_right(), buff=0.08,
                  stroke_width=3.4, color=C_INDICE,
                  max_tip_length_to_length_ratio=0.14)
    g = VGroup(lookup, seek, flecha)
    g.lookup, g.seek, g.flecha = lookup, seek, flecha
    return g


def candado(pos=ORIGIN, color=None, escala=1.0):
    """Un candado sencillo: cuerpo + arco (para 'plan forzado')."""
    color = C_BUENO if color is None else color
    ancho, alto = 0.62 * escala, 0.46 * escala
    cuerpo = RoundedRectangle(width=ancho, height=alto,
                              corner_radius=0.07 * escala, stroke_color=color,
                              stroke_width=2.6, fill_color=color,
                              fill_opacity=0.16)
    arco = Arc(radius=ancho * 0.3, angle=PI, stroke_color=color,
              stroke_width=2.6)
    arco.next_to(cuerpo, UP, buff=-0.04 * escala)
    ojo = Dot(radius=0.04 * escala, color=color)
    ojo.move_to(cuerpo.get_center())
