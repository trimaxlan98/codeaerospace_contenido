# =====================================================================
# CO.DE Academy - "Protocolos de Internet · 1.1 Trocear el mensaje:
# conmutacion de paquetes". Bloque de estilo del proyecto. Se antepone al
# script de CADA clip; los clips NO repiten imports: solo definen su
# ClipN(Scene).
#
# Este bloque es el MOLDE de la familia "Protocolos de Internet": las 23
# lecciones restantes copian este archivo y solo cambian la cabecera y la
# tabla de numeros de su leccion. Por eso la paleta, los rotulos y los
# helpers viven aqui y no en un clip.
# =====================================================================
import math
import sys
...
# La red de la leccion: dos caminos entre A y B.
POS_RED = {"A": (-4.5, 0.0), "R1": (-2.5, 0.0), "R2": (-0.5, 1.35),
           "R3": (-0.5, -1.35), "R4": (1.6, 0.0), "B": (3.7, 0.0)}
ARISTAS_RED = {("A", "R1"): None, ("R1", "R2"): None, ("R1", "R3"): None,
               ("R2", "R4"): None, ("R3", "R4"): None, ("R4", "B"): None}
TIPOS_RED = {"A": "host", "B": "servidor"}
CAMINO_ALTO = ["A", "R1", "R2", "R4", "B"]
CAMINO_BAJO = ["A", "R1", "R3", "R4", "B"]

# Clip 1 - el circuito reservado (1 Mb de mensaje, 10 Mb/s, 3 saltos).
MSG_BITS = 1_000_000
CONM = conmutacion(MSG_BITS, tasa_mbps=10.0, saltos=3, km_por_salto=500.0,
                   establecer_ms=250.0, tam_paquete_bits=12000)
CIRCUITOS_ENLACE = 3          # cuantas llamadas caben reservando
LLAMADAS_PEDIDAS = 5
LLAMADAS_BLOQUEADAS = LLAMADAS_PEDIDAS - CIRCUITOS_ENLACE

# Clip 2 - trocear un mensaje de 12 000 B.
MSG_BYTES = 12_000
TROZO = troceado(MSG_BYTES, mtu=1500, cabeceras=40)
TROZO_CHICO = troceado(100, mtu=1500, cabeceras=40)
ORDEN_LLEGADA = [1, 3, 2, 5, 4, 6, 8, 7, 9]     # llegan desordenados

# Clip 3 - multiplexacion estadistica.
MUX = mux_estadistico(n_flujos=10, pico_mbps=2.0, ciclo_activo=0.15,
                      capacidad_mbps=6.0, pasos=600, semilla=11)
PICO_FLUJO = 2.0
MUX_VENTANA = 60              # pasos que se dibujan de la traza
# Las cifras que se rotulan en el clip 3 se miden sobre la MISMA ventana
# que se dibuja: citar la corrida entera mientras se ve un trozo mentiria.
MUX_D = MUX["demanda"][:MUX_VENTANA]
MUX_MEDIA_V = float(MUX_D.mean())
MUX_EXCESO_V = 100.0 * float((MUX_D > MUX["capacidad_mbps"]).sum()) / MUX_VENTANA


def DEMANDA(k):
    """La demanda medida en el paso k de la ventana dibujada (Mb/s)."""
    i = int(round(k))
    i = min(max(i, 0), MUX_VENTANA - 1)
    return float(MUX["demanda"][i])


# Clip 4 - la cola con bufer finito.
COLA = cola_mm1(lmbda=0.85, mu=1.0, n_llegadas=4000, capacidad=8, semilla=3)
COLA_CAP = COLA["capacidad"]
OCUPACIONES = [0, 1, 2, 1, 3, 4, 6, 7, 8, 8, 6, 4, 2]   # el relato visual
