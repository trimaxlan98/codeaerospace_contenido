# =====================================================================
# CO.DE Academy - "Tesis 6G · 3.1 El termómetro roto". Bloque de estilo
...
LAMBDA = np.degrees(np.arccos(R_T * np.cos(np.radians(EL_MIN))
                              / (R_T + H_KM))) - EL_MIN
D = T6.datos_tesis()
ACIERTO_A, ACIERTO_B, LECTURA = 0.70, 0.72, 37      # la analogia del seminario (no son datos)
T_TR, SIN_AZAR = T6.trayectorias(n=6, azar=0.0, semilla=42)
_, CON_AZAR = T6.trayectorias(n=6, azar=0.85, semilla=42)
INV = T6.semillas_invierten()
CM = T6.curvas_margen()
