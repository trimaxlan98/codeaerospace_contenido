# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 3.2 Índices que no se ven".
...
PENDIENTES = S.filas_estatus("pendiente")                       # 2,791
FRAC_PENDIENTES = S.fraccion_estatus()["pendiente"] * 100        # 0.186 -> "0.19 %"
DETALLE_TOTAL = S.detalle_total()                                # 3,750,000
PAG_DETALLE = M["paginas_detalle"]                               # 17,606 (motor)
TICKET_FILAS = int(S.lineas_detalle(750000))                     # 1 (renglones del pedido)
RAZON_TICKET = S.razon(M["ticket_sin_indice"], M["ticket_con_indice"])  # ~6,005
...
def celdas_pendientes(total):
    """Indices de celda (de `total`, mismo orden que MuroPaginas: por
    renglon) que contienen al menos un pedido pendiente. `pedidos()` esta
    ordenado por fecha: los pendientes solo salen entre los pedidos MAS
    RECIENTES (la tabla crece por fecha), asi que por construccion de los
    datos caen en las ULTIMAS celdas del muro. No es un adorno: se cuenta
    sobre la reproduccion real."""
    p = S.pedidos()
    n = len(p["estatus"])
    tam = n // total
    pend = p["estatus"] == S.ESTATUS.index("pendiente")
    return [i for i in range(total) if pend[i * tam:(i + 1) * tam].any()]
