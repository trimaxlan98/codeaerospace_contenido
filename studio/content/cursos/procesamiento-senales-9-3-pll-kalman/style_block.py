# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 9.3 PLL y Kalman".
...
# --- Numeros de la leccion --------------------------------------------
# Un tono cuya frecuencia se va (el Doppler de un pase): lo que hay que
# seguir.
N_PLL = 1500
F_INI, DERIVA = 0.02, 1.2e-5
X_PLL, F_REAL = tono_con_deriva(N_PLL, F_INI, DERIVA, 0.05, 5)
KP, KI = 0.05, 0.002
FASE_PLL, ERR_PLL, FREC_PLL = pll(X_PLL, KP, KI, F_INI)
N_ENGANCHE = 600
ERR_RMS_PLL = float(np.sqrt(np.mean(ERR_PLL[N_ENGANCHE:] ** 2)))   # 0.055 rad
FREC_FINAL = float(FREC_PLL[-1])          # 0.037975
FREC_REAL_FINAL = float(F_REAL[-1])       # 0.037988
ERR_FREC = abs(FREC_FINAL - FREC_REAL_FINAL)                       # 1.3e-05

# Kalman: cuanto creerse cada medida
N_KAL = 300
VERDAD, MEDIDAS = medida_ruidosa(N_KAL, 0.6, 3)
R_KAL = 0.36
QS = (1e-4, 1e-2)
KAL = {q: kalman_escalar(MEDIDAS, q, R_KAL) for q in QS}
EST = {q: KAL[q][0] for q in QS}
GANANCIA = {q: KAL[q][1] for q in QS}
RMSE_MEDIDAS = rmse(MEDIDAS, VERDAD)      # 0.6081
RMSE_KAL = {q: rmse(EST[q], VERDAD) for q in QS}   # 0.2385 / 0.1899
GAN_FINAL = {q: float(GANANCIA[q][-1]) for q in QS}   # 0.0165 / 0.1534
MEJORA_KAL = RMSE_MEDIDAS / RMSE_KAL[1e-2]
