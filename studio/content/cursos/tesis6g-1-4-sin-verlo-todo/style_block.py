# =====================================================================
# CO.DE Academy - "Tesis 6G · 1.4 Decidir sin verlo todo". Bloque de estilo
...
E = T6.entorno_v2()
D = T6.datos_tesis()
ESTADO, OBS = T6.ESTADO_V2, T6.OBS_V2
CANAL = T6.canal_degradado_v2(3 * E["canal_periodo"], E)
Y_OBS, CREENCIA = T6.creencia_canal(T6.interferencia_v2(CANAL == 0, E))
_cambios = np.nonzero(np.diff(CANAL))[0] + 1


def _retraso(k):
    objetivo = CANAL[k] == 0
    j = k
    while j < len(CANAL) and (CREENCIA[j] > 0.5) != objetivo:
        j += 1
    return j - k


RETRASOS = [_retraso(k) for k in _cambios]            # pasos en darse cuenta
ACIERTO = float(((CREENCIA > 0.5) == (CANAL == 0)).mean())
N_AG, N_ACC = E["agentes"], 3
N_CONJ = N_ACC ** N_AG                                # 27
MEJOR_FIJA = D["g2b"][0]                              # best_joint de G1/G2b
G1_42 = T6._leer("g1_42.json")
TOP5 = G1_42["top5"]                                   # las 5 mejores fijas, semilla 42
HORIZ = [1, 2, 3, 5, 10]
LOG_POL = [T6.log10_politicas_dec(N_AG, N_ACC, 2, h) for h in HORIZ]
