# =====================================================================
# CO.DE Academy - "Tesis 6G · 3.4 Lo que ya se encontró". Bloque de estilo
...
D = T6.datos_tesis()
G = D["g2b"]
MA_DEC = min(g["mejora"] for g in G)
COMP = {c["id"]: c for c in D["compuertas"]}
ORDEN = ["G0", "G1", "G2a", "G2b", "G3", "G4"]
import re as _re
_g2a = _re.findall(r"\d+(?:\.\d+)?", COMP["G2a"]["valor"])[:2]
VALOR = {"G0": (T6.pct(D["g0_ma"]), "no"), "G1": (T6.pct(D["g1_ma"]), "ok"),
         "G2a": (f"+{_g2a[0]}-{_g2a[1]} %", "ok"),
         "G2b": (f"+{100 * min(g['mejora'] for g in G):.1f}-{100 * max(g['mejora'] for g in G):.1f} %", "ok")}
