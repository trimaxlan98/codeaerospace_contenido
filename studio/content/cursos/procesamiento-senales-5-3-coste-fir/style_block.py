# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 5.3 Cuanto cuesta un FIR".
...
ATEN_PEDIDA = 45.0
ORDEN = 40
H = fir_equirriple(ORDEN, F_PASO, F_RECHAZO, FS_D)
N_TAPS = len(H)                                # 41
SIMETRICO = es_simetrico(H)                    # True
MACS = macs_fir(H)                             # 21 (la mitad + 1)
_, ATEN = rizado_db(H, [1.0], F_PASO, F_RECHAZO, FS_D)

# el retardo, MEDIDO metiendo un impulso y viendo donde sale
N_PRUEBA = 60
POS_IMPULSO = 10
X_PRUEBA = np.zeros(N_PRUEBA)
X_PRUEBA[POS_IMPULSO] = 1.0
Y_PRUEBA = convolucion(X_PRUEBA, H)
RETARDO = int(np.argmax(np.abs(Y_PRUEBA))) - POS_IMPULSO   # 20 = N/2

# el filtro corto que se dibuja como estructura (7 taps, simetrico)
H_CORTO = fir_equirriple(6, 0.25, 0.45, FS_D)
PARES_SIMETRICOS = [(i, len(H_CORTO) - 1 - i)
                    for i in range(len(H_CORTO) // 2)]

FS_AUDIO = 48000
MMAC_S = MACS * FS_AUDIO / 1e6                 # 1.008 MMAC/s
FS_ALTA = 192000
MMAC_S_ALTA = MACS * FS_ALTA / 1e6             # 4.032
