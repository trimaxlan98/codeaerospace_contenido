# =====================================================================
# CO.DE Academy - "Tesis 6G · 2.2 Muchos agentes". Bloque de estilo
...
LAMBDA = np.degrees(np.arccos(R_T * np.cos(np.radians(EL_MIN))
                              / (R_T + H_KM))) - EL_MIN
SEM_CLIP = 89                                    # semilla de la MEDIANA (sonda)
ACC_IND, COL_IND = T6.aprendices_independientes(semilla=SEM_CLIP)
ACC_CON, COL_CON = T6.aprendices_independientes(semilla=SEM_CLIP, conjunto=True)
IND, CON = T6.coordinacion_estadistica(1000)
MED_IND, MED_CON = float(np.median(IND)), float(np.median(CON))
P90_IND, P90_CON = float(np.percentile(IND, 90)), float(np.percentile(CON, 90))
_rng = np.random.default_rng(1)
IGM_FALLOS = {t: 0 for t in ("vdn", "qmix", "roto")}
for _ in range(500):
    _q1, _q2 = _rng.uniform(-1, 1, 3), _rng.uniform(-1, 1, 3)
    for _t in IGM_FALLOS:
        IGM_FALLOS[_t] += not T6.igm(_q1, _q2, _t)[0]
VQ = T6.vdn_contra_qmix()
