# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 7.3 La tienda esta lenta".
...
M = S.MEDIDO
PETICIONES = M["caso_peticiones"]                                # 300 (motor)
ANTES = M["caso_antes_s"]                                        # 53.0 s (motor)
DESPUES = M["caso_despues_s"]                                    # 3.9 s (motor)
RAZON = S.razon(ANTES, DESPUES)                                  # ~13.6x (calculado)
...
def txt_seg(x):
    """'53 segundos' o '3.9 segundos' (motor_pie): un decimal solo si
    hace falta (53.0 no es 53.0 segundos, es 53 segundos)."""
    d = 0 if float(x).is_integer() else 1
    return f"{fmt(x, d)} segundos"


def txt_seg_corto(x):
    """'53 s' o '3.9 s' (tag_motor, junto a una barra: menos espacio)."""
    d = 0 if float(x).is_integer() else 1
    return f"{fmt(x, d)} s"


def icono_llave(color=None, escala=1.0):
    """Una llave (cabeza + vastago + dos dientes): la llave foranea sin
    indice (el ticket)."""
    col = C_MALO if color is None else color
    cabeza = Circle(radius=0.22 * escala, stroke_color=col, stroke_width=3)
    vastago = Line(cabeza.get_right(), cabeza.get_right() + RIGHT * 0.48 * escala,
                   stroke_color=col, stroke_width=3)
    d1 = Line(vastago.get_end(), vastago.get_end() + DOWN * 0.16 * escala,
             stroke_color=col, stroke_width=3)
    d2 = Line(vastago.get_end() + LEFT * 0.15 * escala,
             vastago.get_end() + LEFT * 0.15 * escala + DOWN * 0.11 * escala,
             stroke_color=col, stroke_width=3)
    return VGroup(cabeza, vastago, d1, d2)


def icono_viaje(color=None, escala=1.0):
    """Dos cajas con una flecha que sale y otra que vuelve: el viaje de
    vuelta de un lookup (mis pedidos, antes del indice que cubre)."""
    col = C_MALO if color is None else color
    a = Rectangle(width=0.36 * escala, height=0.36 * escala, stroke_color=col,
                 stroke_width=2.4)
    b = Rectangle(width=0.36 * escala, height=0.36 * escala, stroke_color=col,
                 stroke_width=2.4)
    b.next_to(a, RIGHT, buff=0.5 * escala)
    ida = Arrow(a.get_right() + UP * 0.1 * escala, b.get_left() + UP * 0.1 * escala,
               buff=0.05, stroke_width=2.0, color=col,
               max_tip_length_to_length_ratio=0.35)
    vuelta = Arrow(b.get_left() + DOWN * 0.1 * escala,
                  a.get_right() + DOWN * 0.1 * escala, buff=0.05,
                  stroke_width=2.0, color=col, max_tip_length_to_length_ratio=0.35)
    return VGroup(a, b, ida, vuelta)


def icono_tipo(color=None, escala=1.0):
    """N' tachado: el prefijo NVARCHAR sobre una columna VARCHAR (login)."""
    col = C_MALO if color is None else color
    t = Text("N'", font=FUENTE_HUD, font_size=int(30 * escala), color=col)
    tacha = Line(t.get_corner(UP + LEFT) + LEFT * 0.05,
                t.get_corner(DOWN + RIGHT) + RIGHT * 0.05, stroke_color=col,
                stroke_width=2.6)
    return VGroup(t, tacha)


def icono_year(color=None, escala=1.0):
    """YEAR( tachado: la columna envuelta en una funcion (el reporte)."""
    col = C_MALO if color is None else color
    t = Text("YEAR(", font=FUENTE_HUD, font_size=int(24 * escala), color=col)
    tacha = Line(t.get_corner(UP + LEFT) + LEFT * 0.05,
                t.get_corner(DOWN + RIGHT) + RIGHT * 0.05, stroke_color=col,
                stroke_width=2.6)
    return VGroup(t, tacha)
...
def marca_hecho(color=None, escala=1.0):
    """Una palomita: el arreglo ya esta aplicado."""
    col = C_BUENO if color is None else color
    return VGroup(
        Line(LEFT * 0.14 * escala, DOWN * 0.05 * escala, stroke_color=col,
            stroke_width=4),
        Line(DOWN * 0.05 * escala, RIGHT * 0.22 * escala + UP * 0.2 * escala,
