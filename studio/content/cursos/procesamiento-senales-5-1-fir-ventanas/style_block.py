# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 5.1 FIR: truncar el
# ideal". Bloque de estilo del proyecto: se antepone al script de
...
# --- Numeros de la leccion --------------------------------------------
# El pliego de condiciones del LOTE ENTERO (modulos 5 y 6), para poder
# comparar un diseño con otro: paso hasta 0.20 pi, rechazo desde 0.32 pi,
# -45 dB. fs = 2 hace que las frecuencias se lean en fracciones de pi.
FS_D = 2.0
F_PASO, F_RECHAZO = 0.20, 0.32
ATEN_PEDIDA = 45.0
FC = 0.25                      # el corte del ideal que se trunca
ORDEN = 40

# --- el ideal y su recorte -------------------------------------------
N_SINC = 61                    # muestras de la sinc que se dibujan
H_SINC = ideal_truncado(N_SINC - 1, FC, FS_D)
H_IDEAL = ideal_truncado(ORDEN, FC, FS_D)          # 41 taps
N_TAPS = len(H_IDEAL)
W_ID, MAG_ID, _ = respuesta_frec(H_IDEAL, [1.0], 2048)
SOBREPICO_ID, F_SOBREPICO = gibbs_db(H_IDEAL, FC, FS_D)   # +0.82 dB
RIZADO_ID, ATEN_ID = rizado_db(H_IDEAL, [1.0], F_PASO, F_RECHAZO, FS_D)

# --- Gibbs no se va: subir el orden estrecha la transicion, nada mas --
ORDENES = (20, 40, 80, 160)
H_ORDEN = {o: ideal_truncado(o, FC, FS_D) for o in ORDENES}
SOBREPICO = {o: gibbs_db(H_ORDEN[o], FC, FS_D)[0] for o in ORDENES}
ATEN_ORDEN = {o: rizado_db(H_ORDEN[o], [1.0], F_PASO, F_RECHAZO, FS_D)[1]
              for o in ORDENES}
# 0.609 / 0.817 / 0.778 / 0.761 dB de sobrepico; -19.6 / -27.5 / -30.5 /
# -36.4 dB de atenuacion: la oreja se queda, la transicion mejora.

# --- la ventana que ablanda (todas al MISMO orden 40) -----------------
VENTANAS_FIR = ("rect", "hann", "hamming", "blackman")
H_VENT = {v: fir_ventana(ORDEN, FC, v, FS_D) for v in VENTANAS_FIR}
RESP_VENT = {v: respuesta_frec(H_VENT[v], [1.0], 2048) for v in VENTANAS_FIR}
SOBREPICO_V = {v: gibbs_db(H_VENT[v], FC, FS_D)[0] for v in VENTANAS_FIR}
RIZADO_V, ATEN_V = {}, {}
for _v in VENTANAS_FIR:
    RIZADO_V[_v], ATEN_V[_v] = rizado_db(H_VENT[_v], [1.0], F_PASO,
                                         F_RECHAZO, FS_D)
# sobrepico 0.817 -> 0.002 dB; atenuacion -27.5 / -33.7 / -35.6 / -24.1:
# blackman mata a Gibbs pero su lobulo se come la transicion y SUSPENDE.

# --- lo que cuesta cumplir el pliego ----------------------------------
ORDEN_VENTANA = 72             # medido probando (hann y blackman)
H_PLIEGO_VENT = fir_ventana(ORDEN_VENTANA, (F_PASO + F_RECHAZO) / 2, "hann",
                            FS_D)
_, ATEN_PLIEGO_VENT = rizado_db(H_PLIEGO_VENT, [1.0], F_PASO, F_RECHAZO,
                                FS_D)
MACS_VENTANA = macs_fir(H_PLIEGO_VENT)             # 37
ORDEN_EQ = 40
H_EQ = fir_equirriple(ORDEN_EQ, F_PASO, F_RECHAZO, FS_D)
RIZADO_EQ, ATEN_EQ = rizado_db(H_EQ, [1.0], F_PASO, F_RECHAZO, FS_D)
MACS_EQ = macs_fir(H_EQ)                           # 21
W_EQ, MAG_EQ, _ = respuesta_frec(H_EQ, [1.0], 2048)
