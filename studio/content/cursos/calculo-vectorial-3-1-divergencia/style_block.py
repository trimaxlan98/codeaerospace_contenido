# =====================================================================
# CO.DE Academy - "Cálculo vectorial · 3.1 La divergencia: fuentes y sumideros".
...
LADO_CAJA = 1.2                          # lado de la cajita contable (clips 1-3)
P_CAJA = np.array([1.3, 0.7])            # punto de la cajita (clips 1 y 3)
CAMPO_FUENTE = campo_radial              # F=(x,y): div=2 en todo punto
CAMPO_SUMIDERO = lambda p: -campo_radial(p)   # el radial invertido: div=-2
CAMPO_NEUTRO = campo_rotor               # F=(-y,x): div=0, solo gira
P_TRIO = np.array([0.9, 0.5])            # punto comun de las tres cajitas (clip 2)

# El mapa (clip 4): campo con divergencia VARIABLE punto a punto -- no la
# div constante del remolino amortiguado (que seria sumidero en TODO el
# plano). div(campo_mapa) = cos(x) + cos(y): fuente cerca del origen,
# sumidero cerca de (pi, pi) y equivalentes.
CAMPO_MAPA = lambda p: np.array([np.sin(p[0]), np.sin(p[1])])
P_FUENTE_MAPA = np.array([0.0, 0.0])         # div = cos0+cos0 = +2.0
P_SUMIDERO_MAPA = np.array([2.4, 2.4])       # div = cos2.4+cos2.4 < 0
MALLA_MAPA = dict(x0=-3.4, x1=3.4, y0=-2.6, y1=2.6, paso=0.85)
RADIO_SIEMBRA = 1.3                          # anillo de particulas alrededor de la fuente
N_SIEMBRA = 8                                # numero de particulas sembradas
