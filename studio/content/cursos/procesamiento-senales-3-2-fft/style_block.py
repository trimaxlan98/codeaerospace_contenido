# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 3.2 La FFT: partir en dos".
...
# --- Numeros de la leccion --------------------------------------------
N_FFT = 8                          # el grafo que se dibuja
X_FFT = np.array([0.8, -0.4, 0.9, 0.3, -0.7, 0.5, 0.2, -0.6])
ETAPAS = mariposas(N_FFT)          # 3 etapas x 4 mariposas
N_ETAPAS = len(ETAPAS)             # 3 = log2(8)
POR_ETAPA = len(ETAPAS[0])         # 4 = N/2
ORDEN = bit_reverso(N_FFT)         # [0, 4, 2, 6, 1, 5, 3, 7]
ESTADOS = fft_por_etapas(X_FFT)    # 4 estados (entrada + 3 etapas)
X_FINAL = ESTADOS[-1]
ERROR_FFT = float(np.abs(X_FINAL - dft(X_FFT)).max())   # 2.4e-15
# (contra la DFT MATRICIAL, que es la que se dibuja; contra np.fft
#  el error es 1.1e-16 — la matriz acumula mas redondeo)

# partir en dos: la DFT de 8 son dos DFT de 4 combinadas
PARES = X_FFT[0::2]
IMPARES = X_FFT[1::2]
X_PARES = dft(PARES)
X_IMPARES = dft(IMPARES)
TWIDDLE = np.exp(-2j * np.pi * np.arange(N_FFT // 2) / N_FFT)
X_RECOMPUESTA = np.concatenate([X_PARES + TWIDDLE * X_IMPARES,
                                X_PARES - TWIDDLE * X_IMPARES])
ERROR_PARTIR = float(np.abs(X_RECOMPUESTA - dft(X_FFT)).max())   # 2.2e-15

# el precio, CONTADO sobre el grafo (no una formula de libro)
N_GRANDE = 1024
OPS_DFT = ops_dft(N_GRANDE)        # 1048576
OPS_FFT = ops_fft(N_GRANDE)        # 5120
RAZON = OPS_DFT / OPS_FFT          # 204.8
LOG2_GRANDE = int(math.log2(N_GRANDE))
TABLA_N = [8, 64, 1024, 4096]
TABLA_DFT = [ops_dft(n) for n in TABLA_N]
TABLA_FFT = [ops_fft(n) for n in TABLA_N]
TABLA_RAZON = [d / f for d, f in zip(TABLA_DFT, TABLA_FFT)]
