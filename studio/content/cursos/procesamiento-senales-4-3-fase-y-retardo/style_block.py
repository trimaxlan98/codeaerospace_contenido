# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 4.3 Fase, retardo y fase minima".
...
# --- Numeros de la leccion --------------------------------------------
H_SIM = np.ones(9) / 9                        # media movil simetrica
W_SIM, MAG_SIM, FASE_SIM = respuesta_frec(H_SIM, n=512)
PRIMER_NULO = 2 * np.pi / 9                   # 0.698 rad = 0.222 pi
EN_BANDA = W_SIM < PRIMER_NULO * 0.9
AJUSTE = np.polyfit(W_SIM[EN_BANDA], FASE_SIM[EN_BANDA], 1)
PENDIENTE_FASE = float(AJUSTE[0])             # -4.0000 exacto
RESIDUO_FASE = float(np.abs(FASE_SIM[EN_BANDA]
                            - np.polyval(AJUSTE, W_SIM[EN_BANDA])).max())
W_GD, GD_SIM = retardo_grupo(H_SIM)
GD_BANDA = GD_SIM[W_GD < PRIMER_NULO * 0.9]
GD_MIN, GD_MAX = float(GD_BANDA.min()), float(GD_BANDA.max())   # 4.0 y 4.0
# La fase solo es una recta DENTRO de la banda de paso: en los nulos da
# saltos de pi. Medirla en toda la banda da un residuo de 1.7 rad y una
# conclusion falsa.

CEROS_NOMIN = [0.5, 2.0]                      # uno dentro, otro FUERA
B_NOMIN = np.real(np.poly(CEROS_NOMIN))       # [1, -2.5, 1]
B_MIN = reflejar_ceros(B_NOMIN)               # [2, -2, 0.5]
W_N, MAG_NOMIN, FASE_NOMIN = respuesta_frec(B_NOMIN)
_, MAG_MIN, FASE_MIN = respuesta_frec(B_MIN)
DIF_MAG = float(np.abs(MAG_NOMIN - MAG_MIN).max())      # 1.2e-14 dB
H_NOMIN = respuesta_impulso(B_NOMIN, [1.0], 6)
H_MIN = respuesta_impulso(B_MIN, [1.0], 6)
ENERGIA_NOMIN = float(H_NOMIN[0] ** 2 / np.sum(H_NOMIN ** 2) * 100)   # 12.1 %
ENERGIA_MIN = float(H_MIN[0] ** 2 / np.sum(H_MIN ** 2) * 100)         # 48.5 %
GD_NOMIN = float(np.mean(retardo_grupo(B_NOMIN)[1]))    # 1.000
GD_MIN2 = float(np.mean(retardo_grupo(B_MIN)[1]))       # 0.001

ECO_BUENO = np.array([1.0, 0.6])              # eco de fase minima
ECO_MALO = np.array([1.0, 1.4])               # eco que no se puede deshacer
INV_BUENO = respuesta_impulso([1.0], ECO_BUENO, 40)
INV_MALO = respuesta_impulso([1.0], ECO_MALO, 40)
COLA_BUENA = float(abs(INV_BUENO[-1]))        # 2.2e-09: se apaga
COLA_MALA = float(abs(INV_MALO[-1]))          # 5.0e+05: revienta
