# =====================================================================
# CO.DE Academy - "Cálculo vectorial · 2.2 Líneas de flujo: seguir la corriente".
...
CAMPO_VIENTO = campo_viento               # campo de los clips 1 y 2
CAMPO_REMOLINO = campo_remolino           # campo del clip 3 (familia entera)
P0_PARTICULA = np.array([-3.6, -1.7])     # semilla de la particula (clip 1)
T_FLUJO = 7.5                             # duracion de integracion RK4 (clip 1)
P0_EULER = np.array([-2.8, -1.3])         # semilla del paso a paso (clip 2):
                                          # aqui el campo curva lo bastante
                                          # para que el paso grande se note
DT_EULER = 0.7                            # paso GRANDE de Euler a mano (clip 2)
PASOS_EULER = 10                          # numero de pasos de Euler (clip 2)
T_COMPARA = DT_EULER * PASOS_EULER        # mismo tiempo total, RK4 fino (clip 2)

# anillo de semillas alrededor del origen: la familia entera del remolino
SEMILLAS_REMOLINO = [
    (-3.2, 2.0), (-1.6, 2.6), (0.0, 3.0), (1.8, 2.4), (3.2, 1.4),
    (3.4, -1.0), (2.0, -2.6), (0.0, -3.0), (-2.0, -2.4), (-3.4, -0.8),
]
T_REMOLINO = 14.0                         # tiempo de integracion (clip 3)

# retratos del clip 4: radial (rectas), rotor (circulos), silla (hiperbolas)
_ANGULOS_8 = np.linspace(0, 2 * np.pi, 8, endpoint=False)
SEMILLAS_RADIAL = [(0.35 * np.cos(a), 0.35 * np.sin(a)) for a in _ANGULOS_8]
T_RADIAL = 3.2

RADIOS_ROTOR = [0.6, 1.2, 1.8, 2.4, 3.0, 3.4]
SEMILLAS_ROTOR = [(r, 0.0) for r in RADIOS_ROTOR]
T_ROTOR = 6.3                             # ~ 2*pi: una vuelta completa

# silla F=(x,-y): x*y es invariante del flujo, asi que una semilla LEJOS
# de los ejes traza de verdad una rama de hiperbola (no una recta que se
# arrima enseguida al eje x, como pasa si se siembra cerca del origen).
# Dos semillas en el eje x dibujan las separatrices rectas (la "cruz");
# cuatro semillas cerca del eje y, altas, dibujan las cuatro ramas.
SEMILLAS_SILLA = [
    (0.35, 0.0), (-0.35, 0.0),
    (0.4, 2.6), (0.4, -2.6), (-0.4, 2.6), (-0.4, -2.6),
]
T_SILLA = 3.0


def euler_puntos(F, p0, dt, pasos):
    """Poligonal de Euler p_{k+1} = p_k + dt*F(p_k), a mano: el paso
    GRANDE que se compara contra la RK4 fina de la libreria (clip 2)."""
    p = np.array(p0, dtype=float)
    pts = [p.copy()]
    for _ in range(pasos):
        p = p + dt * np.asarray(F(p), dtype=float)
        pts.append(p.copy())
    return pts
