# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 4.2 Del plano a la respuesta".
...
# --- Numeros de la leccion --------------------------------------------
W0 = np.pi / 4
RADIOS = (0.70, 0.92, 0.98)
RES = {r: resonador(r, W0) for r in RADIOS}
PZ = {r: zpk(*RES[r]) for r in RADIOS}
W_EJE, MAG = {}, {}
for r in RADIOS:
    W_EJE[r], MAG[r], _ = respuesta_frec(*RES[r], n=1024)
PICO = {r: float(MAG[r].max()) for r in RADIOS}          # 8.9 / 19.3 / 31.0
ANCHO_3DB = {r: float(np.sum(MAG[r] > MAG[r].max() - 3.0) / len(MAG[r]))
             for r in RADIOS}                            # 0.335 / 0.053 / 0.013

R_DEMO = 0.92
CEROS_D, POLOS_D, K_D = PZ[R_DEMO]
W_DEMO = W0
MOD_DIST, DIST_CEROS, DIST_POLOS = por_distancias(CEROS_D, POLOS_D, K_D,
                                                  W_DEMO)
MOD_FORMULA = float(abs(h_en(*RES[R_DEMO], W_DEMO)))
# 9.1991 por distancias y 9.1991 por formula: la construccion geometrica
# NO es una analogia, es la cuenta.
D_POLO_CERCA = float(DIST_POLOS.min())        # 0.08: el polo casi pegado
W_MUESTRA = (0.0, np.pi / 8, np.pi / 4, np.pi / 2, 3 * np.pi / 4, np.pi)
MOD_EN = [por_distancias(CEROS_D, POLOS_D, K_D, w)[0] for w in W_MUESTRA]

B_NOTCH, A_NOTCH = notch(0.97, 2 * np.pi * 0.2)
W_NOTCH, MAG_NOTCH, _ = respuesta_frec(B_NOTCH, A_NOTCH, 2048)
MIN_NOTCH = float(MAG_NOTCH.min())
W_MIN_NOTCH = float(W_NOTCH[int(np.argmin(MAG_NOTCH))])
