# =====================================================================
# CO.DE Academy - "Tesis 6G · 2.4 La arquitectura PADA". Bloque de estilo
...
LAMBDA = np.degrees(np.arccos(R_T * np.cos(np.radians(EL_MIN))
                              / (R_T + H_KM))) - EL_MIN
D = T6.datos_tesis()
MODULOS = [("Percepcion", 90), ("Analisis", 0), ("Decision", -90), ("Accion", 180)]
RHO = 0.9                                            # correlacion ELEGIDA del juguete
IB_BITS = np.linspace(0, 4, 200)
IB_REL = T6.cuello_gaussiano(IB_BITS, RHO)
IB_MAX = T6.info_mutua_gauss(RHO)
IB_2 = float(T6.cuello_gaussiano(2.0, RHO) / IB_MAX)  # fraccion con 2 bits
RTT_CENIT = 2 * ntn.retardo_ida_ms(H_KM)                           # 4.0 ms
RTT_BORDE = 2 * ntn.retardo_ida_ms(ntn.distancia_oblicua_km(EL_MIN, H_KM))   # 12.9 ms


def en_circulo(c, r, g):
    a = np.radians(g)
    return c + r * np.array([np.cos(a), np.sin(a), 0.0])


def lazo_pada(c, r, rn, color, ancho=4, discontinuo=False):
    nodos, flechas = VGroup(), VGroup()
    for _, g in MODULOS:
        nodos.add(Circle(radius=rn, stroke_color=color, stroke_width=ancho, fill_color=CODE_BG,
                         fill_opacity=1.0).move_to(en_circulo(c, r, g)))
    hueco = np.degrees(rn / r) + 6
    for _, g in MODULOS:
        a = Arc(radius=r, start_angle=np.radians(g - hueco), angle=-np.radians(90 - 2 * hueco),
                arc_center=c, stroke_color=color, stroke_width=ancho)
        a.add_tip(tip_length=0.2, tip_width=0.2)
