# =====================================================================
# CO.DE Academy - "Tesis 6G · 3.2 Lo que el margen acota". Bloque de estilo
...
LAMBDA = np.degrees(np.arccos(R_T * np.cos(np.radians(EL_MIN))
                              / (R_T + H_KM))) - EL_MIN
D = T6.datos_tesis()
MA_DEC = min(g["mejora"] for g in D["g2b"])    # cota inferior demostrada (peor semilla G2b)
MA_G1 = D["g1_ma"]                             # envolvente estimada (G1), tambien cota inferior
PLANO = T6.curvas_margen(plano=True)
_VIS = PLANO["R"][0] > np.median(PLANO["R"][0])
ADAPT = np.where(_VIS, PLANO["R"][0], PLANO["R"][2])
J3, J8 = T6.juego_coordinacion(3), T6.juego_coordinacion(8)
