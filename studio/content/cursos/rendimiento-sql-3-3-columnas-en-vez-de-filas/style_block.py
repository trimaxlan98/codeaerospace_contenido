# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 3.3 Columnas en vez de filas".
...
M = S.MEDIDO
DET = S.detalle_total()                                          # 3,750,000
N_RG = S.rowgroups()                                              # 4
CAP_RG = S.FILAS_ROWGROUP                                         # 1,048,576 (dato)
ULTIMO_RG = DET - CAP_RG * (N_RG - 1)                             # 604,272
FRAC_ULTIMO = ULTIMO_RG / CAP_RG                                  # ~0.576 (parcial)
FILAS_LOTE = S.FILAS_LOTE                                         # 900 (dato, aprox)
LOTES = math.ceil(DET / FILAS_LOTE)                               # ~4,167
CS_FILAS = M["cs_filas"]                                          # 16,468 (motor)
CS_1HILO = M["cs_lob_1hilo"]                                      # 6,512 (motor, 1 hilo)
CS_8HILOS = M["cs_lob_8hilos"]                                    # 24,700 (motor, 8 hilos)
RAZON_CS = S.razon(CS_FILAS, CS_1HILO)                            # ~2.53
...
def celda_columna(nombre, color=C_TENUE, alto=0.6, fs=20, relleno=0.14):
    """Una celda de detalle_pedido con su nombre real: el ancho se ajusta
    al TEXTO (nunca se encoge la letra). Letra >= 20 px (regla del clip 1)."""
    etq = Text(nombre, font_size=fs, color=C_TITULO)
    ancho = etq.width + 0.32
    caja = Rectangle(width=ancho, height=alto, stroke_color=color,
                     stroke_width=1.8, fill_color=color, fill_opacity=relleno)
    etq.move_to(caja)
    g = VGroup(caja, etq)
    g.caja = caja
    return g
...
def fila_registro(colores=None, alto=0.6, fs=20, buff=0.1):
    """Una fila de detalle_pedido guardada POR FILAS: sus 4 columnas
    completas, una junto a otra (mezcladas en la misma pagina)."""
    colores = colores or {}
    celdas = [celda_columna(n, colores.get(n, C_TENUE), alto, fs)
             for n in COLUMNAS]
    fila = VGroup(*celdas)
...
def segmento_columna(nombre, color=C_TENUE, alto=1.7, fs=20, relleno=0.14):
    """Un segmento columnstore: guarda SOLO esa columna (el almacen POR
    COLUMNAS). Las rayas sugieren la compresion; el rotulo mide lo mismo
    que la caja, asi que nunca se monta con el vecino."""
    etq = Text(nombre, font_size=fs, color=C_TITULO)
    ancho = etq.width + 0.32
    caja = RoundedRectangle(width=ancho, height=alto, corner_radius=0.06,
                            stroke_color=color, stroke_width=1.8,
                            fill_color=color, fill_opacity=relleno)
    rayas = VGroup(*[Line(caja.get_left() + RIGHT * 0.1 + UP * y,
                          caja.get_right() + LEFT * 0.1 + UP * y,
                          stroke_color=color, stroke_width=1.1,
                          stroke_opacity=0.4)
                     for y in np.arange(-alto / 2 + 0.16, alto / 2 - 0.12, 0.18)])
    etq.next_to(caja, DOWN, buff=0.14)
    g = VGroup(caja, rayas, etq)
    g.caja = caja
    return g


def grupo_filas(ancho, alto, frac=1.0, color=C_INDICE, relleno=0.14):
    """Un rowgroup: el ANCHO es proporcional a cuantas filas trae (frac=1.0
    lleno; frac<1.0 parcial, como el ultimo grupo)."""
    return RoundedRectangle(width=max(ancho * frac, 0.35), height=alto,
                            corner_radius=0.05, stroke_color=color,
