# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 6.3 Parametros opcionales".
...
# --- 6.3: parametros opcionales (catch-all) ----------------------------
CATCHALL_SIN = M["catchall_sin_oppo"]                           # 20,010 (motor)
OPPO_VAR = M["oppo_variantes"]                                  # 2 (motor)
CATCHALL_OPPO = M["catchall_oppo"]                              # 6 (motor)
CATCHALL_OTROS = M["catchall_otros"]                            # 20,005 (motor)
DINAMICO = M["dinamico"]                                        # 6 (motor)
RAZON_OPPO = S.razon(CATCHALL_SIN, CATCHALL_OPPO)               # ~3,335 (calculo)
...
def tres_llamadas(font_size=22, buff=0.75):
    """Las 3 llamadas al MISMO procedimiento, cada una con un solo filtro
    presente (los otros dos quedan NULL por omision): solo cliente, solo
    estatus, solo fecha. `font_size` >= 22 (letra minima en TODO texto,
    tambien en S.codigo). `buff` grande por defecto: deja sitio para un
    tag_junto() DEBAJO de cada panel sin que invada el panel siguiente.
    Usada en el clip 1 de esta leccion."""
    paneles = VGroup(
        S.codigo(["EXEC buscar_pedidos", "  @id_cliente = 501"],
                 font_size=font_size),
        S.codigo(["EXEC buscar_pedidos", "  @estatus = 'pendiente'"],
                 font_size=font_size),
        S.codigo(["EXEC buscar_pedidos", "  @desde = '2026-01-01'"],
                 font_size=font_size),
    )
    paneles.arrange(DOWN, buff=buff, aligned_edge=LEFT)
    return paneles
