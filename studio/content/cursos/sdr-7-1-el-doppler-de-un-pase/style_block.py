# =====================================================================
# CO.DE Academy - "SDR · 7.1 El Doppler de un pase".
...
F_SAT = 437e6                                      # (dato)
T_P, DOP, TASA = S.doppler_pase(F_SAT)             # s, Hz, Hz/s (550 km, 60 grados)
DOP_MAX = float(DOP.max())                         # 10.11 kHz
TASA_MAX = float(np.abs(TASA).max())               # 123.1 Hz/s
ERR_T = 2.0                                        # reloj 2 s adelantado (param)
_, RESIDUO = S.residuo_seguimiento(ERR_T, f_hz=F_SAT)
RES_MAX = float(np.abs(RESIDUO).max())             # 246.3 Hz
