# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 9.1 Estimar el
# espectro". Bloque de estilo del proyecto: se antepone al script de
...
# --- Numeros de la leccion --------------------------------------------
# Ruido blanco mas un tono: lo que se quiere estimar es el PISO, y el tono
# esta solo para tener una referencia en pantalla.
FS_E = 1000.0
N_E = 4096
F_TONO = 200.0
X_E = (ruido_blanco(N_E, 1.0, 7)
       + 1.2 * np.sin(2 * np.pi * F_TONO * np.arange(N_E) / FS_E))

F_PER, P_PER = periodograma(X_E, FS_E)
DB_PER = a_db(P_PER)
LEJOS_PER = np.abs(F_PER - F_TONO) > 20.0        # fuera del tono
DISP_PER = dispersion(DB_PER[LEJOS_PER])         # 5.61 dB de temblor

# mas datos NO arreglan el periodograma
N_LARGO = 16384
X_LARGO = ruido_blanco(N_LARGO, 1.0, 7)
F_LARGO, P_LARGO = periodograma(X_LARGO, FS_E)
DISP_LARGO = dispersion(a_db(P_LARGO))           # 5.54 dB: igual

NSEG = (256, 512)
WELCH = {n: welch(X_E, FS_E, n) for n in NSEG}
F_W = {n: WELCH[n][0] for n in NSEG}
DB_W = {n: a_db(WELCH[n][1]) for n in NSEG}
TROZOS = {n: WELCH[n][2] for n in NSEG}          # 31 y 15
DISP_W = {n: dispersion(DB_W[n][np.abs(F_W[n] - F_TONO) > 20.0])
          for n in NSEG}                          # 0.75 y 1.09 dB
RESOL = {n: resolucion_welch(n, FS_E) for n in NSEG}    # 3.91 y 1.95 Hz
RESOL_ENTERA = resolucion_welch(N_E, FS_E)              # 0.24 Hz

MEJORA_DISP = DISP_PER / DISP_W[256]             # 7.5 veces menos temblor
PEOR_RESOL = RESOL[256] / RESOL_ENTERA           # 16 veces peor resolucion
