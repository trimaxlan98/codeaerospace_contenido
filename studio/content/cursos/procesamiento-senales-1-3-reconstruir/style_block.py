# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 1.3 Volver al mundo: reconstruir".
...
T_DENSO = np.linspace(TK[6], TK[-7], 900)      # interior: sin los bordes
REC_SINC = reconstruir_sinc(TK, XK, T_DENSO)
REC_ZOH = zoh(TK, XK, T_DENSO)
ERR_SINC = error_rms(vibracion(T_DENSO), REC_SINC)      # 0.0013
ERR_ZOH = error_rms(vibracion(T_DENSO), REC_ZOH)        # 0.1228

DROOP_NYQ = droop_db(FS / 2, FS)               # -3.922 dB
F_ALTA = 220.0
DROOP_ALTA = droop_db(F_ALTA, FS)              # -1.109 dB
F_ZOH = np.linspace(1e-6, 2.0 * FS, 400)
DB_ZOH = np.array([droop_db(f, FS) for f in F_ZOH])

F_BASE, DB_BASE = espectro_analogico(vibracion, FS, banda=380.0, n=4096)
BANDA = banda_ocupada(F_BASE, DB_BASE, -40.0)  # 222.7 Hz
F_IMAGEN = FS - BANDA                          # 577 Hz
OSR_REC = 4
FS_OSR = OSR_REC * FS                          # 3200 Hz
F_IMAGEN_OSR = FS_OSR - BANDA                  # 2977 Hz
FC_REC, ORDEN_REC = 300.0, 2                   # el filtro de reconstruccion
ATEN_IMAGEN = butter_db(F_IMAGEN, FC_REC, ORDEN_REC)          # -11.7 dB
ATEN_IMAGEN_OSR = butter_db(F_IMAGEN_OSR, FC_REC, ORDEN_REC)  # -39.9 dB

SIGMA_JITTER = 50e-12                          # 50 ps rms de reloj
F_JITTER = [10e3, 30e3, 100e3, 300e3, 1e6]
_J = [snr_jitter(f, SIGMA_JITTER) for f in F_JITTER]
SNR_JITTER = [m for m, _t in _J]               # 110.1 100.5 90.1 80.5 70.1
SNR_TEORICA = [t for _m, t in _J]              # identicas: la sim confirma
