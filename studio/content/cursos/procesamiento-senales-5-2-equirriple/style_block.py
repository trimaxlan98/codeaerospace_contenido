# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 5.2 Equirriple".
...
ATEN_PEDIDA = 45.0
ORDEN = 40
# El diseño por ventanas DESPERDICIA margen: donde sobra atenuacion,
# sobra orden.
H_VENT = fir_ventana(ORDEN, (F_PASO + F_RECHAZO) / 2, "hamming", FS_D)
W_V, MAG_V, _ = respuesta_frec(H_VENT, [1.0], 4096)
F_V = W_V / np.pi
EN_RECHAZO = F_V >= F_RECHAZO
MAX_V = float(MAG_V[EN_RECHAZO].max())        # -28.1 dB (lo peor, en el
                                              # borde de la banda)
MIN_V = float(MAG_V[EN_RECHAZO].min())
# CUIDADO: MIN_V (-119.3 dB) es la profundidad de un NULO, y un nulo es un
# cero sobre el circulo: lo que se mide ahi depende de lo fina que sea la
# malla (-122.0 con 2048 puntos, -141.0 con 16384). NO SE ROTULA.
# Lo que si es estable es el nivel de los LOBULOS, que no se mueve ni un
# 0.01 dB al cambiar la malla:
PICOS_LOBULO = alternancias(H_VENT, F_PASO, F_RECHAZO, FS_D)
ALTURAS_LOBULO = [float(MAG_V[int(np.argmin(np.abs(F_V - x)))])
                  for x in PICOS_LOBULO]
NIVEL_SOSTENIDO = float(max(ALTURAS_LOBULO))   # -55.3 dB
DERROCHE = MAX_V - NIVEL_SOSTENIDO             # 27.2 dB de margen sobrante
# El filtro se juzga por su PEOR punto (-28.1 dB), pero en casi toda la
# banda esta 27 dB por debajo de lo que hace falta: ese margen es orden
# que se esta pagando y no se usa.

H_EQ = fir_equirriple(ORDEN, F_PASO, F_RECHAZO, FS_D)
W_EQ, MAG_EQ, _ = respuesta_frec(H_EQ, [1.0], 4096)
RIZADO_EQ, ATEN_EQ = rizado_db(H_EQ, [1.0], F_PASO, F_RECHAZO, FS_D)
ALT = alternancias(H_EQ, F_PASO, F_RECHAZO, FS_D)      # 14 extremos
ALTURAS = [float(MAG_EQ[int(np.argmin(np.abs(F_V - x)))]) for x in ALT]
SPREAD = float(max(ALTURAS) - min(ALTURAS))   # 0.055 dB: todos iguales

ORDEN_45, ATEN_45 = orden_necesario(F_PASO, F_RECHAZO, 45.0, FS_D)  # 40
ORDEN_60, ATEN_60 = orden_necesario(F_PASO, F_RECHAZO, 60.0, FS_D)  # 56
H_60 = fir_equirriple(ORDEN_60, F_PASO, F_RECHAZO, FS_D)
_, MAG_60, _ = respuesta_frec(H_60, [1.0], 4096)
