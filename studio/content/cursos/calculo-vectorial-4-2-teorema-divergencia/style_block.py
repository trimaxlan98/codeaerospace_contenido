# =====================================================================
# CO.DE Academy - "Cálculo vectorial · 4.2 Flujo y el teorema de la divergencia".
...
def normal_exterior(r, t, h=1e-5):
    """n exterior de la cerrada r(t) antihoraria (misma convencion que
    `normales_borde` y que el signo de `flujo_curva`)."""
    d = np.asarray(r(t + h), float) - np.asarray(r(t - h), float)
    d = d / max(1e-12, np.linalg.norm(d))
    return np.array([d[1], -d[0]])


# El campo radial sale en los clips 1 y 2 con mallas distintas: sin una
# referencia FIJA de magnitud, cada malla se normaliza por su propio
# percentil 90 y el mismo campo saldria de otro color en cada clip.
MAG_MAX_RADIAL = float(np.linalg.norm(campo_radial((3.15, 2.25))))   # 3.87

# -- clip 1: el flujo por un circulo en el campo radial ----------------
R_CURVA = 1.5
CURVA_C1 = circulo((0.0, 0.0), R_CURVA)
N_NORMALES = 12                                    # normales dibujadas
T_LECTURAS = (0.0, 1.0 / 12.0, 4.0 / 12.0)         # donde se lee F . n
FN_LECTURAS = [float(np.dot(campo_radial(CURVA_C1(t)),
                            normal_exterior(CURVA_C1, t)))
               for t in T_LECTURAS]                # 1.5 en las tres
PERIMETRO_C1 = 2 * PI * R_CURVA                    # 9.42
FLUJO_C1 = flujo_curva(campo_radial, CURVA_C1)     # 14.14 = 1.5 * 9.42
SEMILLAS_C1 = [(0.55, 0.30), (-0.60, 0.25), (-0.30, -0.55), (0.35, -0.50)]
AVANCE_C1 = 3.6                                    # factor radial del cruce

# -- clip 2: las cajitas que se cancelan -------------------------------
RECT_C2 = (-1.5, 1.5, -1.0, 1.0)                   # x0, x1, y0, y1
LADO_CAJA = 1.0
CENTROS_CAJAS = [(cx, cy) for cy in (-0.5, 0.5)
                 for cx in (-1.0, 0.0, 1.0)]       # malla 3 x 2
I_CAJA_IZQ, I_CAJA_DER = 1, 2                      # las dos que comparten
PARED_X = 0.5                                      # ...esta pared interior
DIV_INTERIOR_C2 = integral_doble(
    lambda p: div_num(campo_radial, p), *RECT_C2, n=80)      # 12.00

# -- clip 3: el cubo en 3D ---------------------------------------------
CUBO_ESQUINA = (0.2, 0.2, 0.2)
CUBO_LADO = 1.6
NOMBRES_CARAS = ("+x", "-x", "+y", "-y", "+z", "-z")   # orden de .flujos
CARAS_ROTULADAS = (0, 4, 1)                        # +x, +z y la de atras
DIV_RADIAL3 = div_num(campo_radial3, (0.7, 0.4, 1.1))    # 3.00 (constante)
VOL_CUBO = CUBO_LADO ** 3                          # 4.10
TRIPLE_C3 = DIV_RADIAL3 * VOL_CUBO                 # 12.29 = flujo total

# -- clip 4: la ley de Gauss -------------------------------------------
# La fuente puntual crece como 1/r: sin referencia fija, el percentil 90 de
# la malla deja calientes (ambar) las flechas de cerca del origen y ya no se
# distinguen de las normales n, que en esta familia son ambar por rol.
MAG_MAX_FUENTE = float(np.linalg.norm(campo_fuente((0.5, 0.0))))   # 2.00
R_GAUSS_A, R_GAUSS_B = 0.9, 1.8                    # el segundo, doble radio
C_FUERA, R_FUERA = (3.5, -1.05), 0.75              # la que NO encierra
FLUJO_GAUSS_A = flujo_curva(campo_fuente, circulo((0.0, 0.0), R_GAUSS_A))
FLUJO_GAUSS_B = flujo_curva(campo_fuente, circulo((0.0, 0.0), R_GAUSS_B))
FLUJO_GAUSS_FUERA = flujo_curva(campo_fuente, circulo(C_FUERA, R_FUERA))
DIV_FUENTE_FUERA = div_num(campo_fuente, C_FUERA)  # 0.00
DOS_PI = 2 * PI                                    # 6.28
