# =====================================================================
# CO.DE Academy - "SDR · 3.1 Sintonizar en software".
...
FS = S.RTL["fs"]
CENTRO = 96.4e6                                    # la captura del 1.1
CAP = S.captura_banda(CENTRO)                      # 16384 muestras complejas
OFFS, NIVS = S.emisoras_captura(CENTRO)            # -700,-300,+100,+900,+1100 kHz
OFF_FUERTE = float(OFFS[int(np.argmax(NIVS))])     # -700 kHz
TRES = (float(OFFS[0]), float(OFFS[2]), float(OFFS[4]))   # -700 / +100 / +1100
ESP_NCO, SEP_NCO = S.espurio_nco_dbc()             # -9.5 dBc a 240 Hz
BLOQUE, FS_NCO, F_NCO = 1000, 240e3, 37.5e3        # parametros (gris)
