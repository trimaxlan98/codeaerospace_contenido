# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 9.2 LMS".
...
# --- Numeros de la leccion --------------------------------------------
# El escenario clasico: una voz limpia, un ruido que se cuela por un camino
# desconocido, y un microfono de referencia con SOLO el ruido.
N_LMS = 4000
D_LMS, REF, LIMPIA, CAMINO = escenario_cancelacion(N_LMS, 11)
RUIDO_ENTRA = D_LMS - LIMPIA
N_TAPS_LMS = len(CAMINO)                  # 5
MU_MAX = mu_maximo(REF, N_TAPS_LMS)       # 0.4001

MUS = (0.02, 0.005, 0.001)
LMS_RES = {m: lms(REF, D_LMS, m, N_TAPS_LMS) for m in MUS}
W_FIN = {m: LMS_RES[m][0] for m in MUS}
E_LMS = {m: LMS_RES[m][1] for m in MUS}
CURVA = {m: curva_aprendizaje(E_LMS[m], LIMPIA) for m in MUS}
CONV = {m: convergencia(CURVA[m]) for m in MUS}   # (n, suelo)
N_CONV = {m: CONV[m][0] for m in MUS}             # 76 / 459 / 2894
SUELO = {m: CONV[m][1] for m in MUS}              # 0.0187 / 0.0041 / 0.0009
MEJORA = {m: mejora_db(RUIDO_ENTRA, E_LMS[m] - LIMPIA) for m in MUS}
ERR_COEF = {m: error_coeficientes(W_FIN[m], CAMINO) for m in MUS}
MU_DEMO = 0.005

# por encima del maximo, revienta
MU_MALO = MU_MAX * 1.02
_, E_MALO, _ = lms(REF, D_LMS, MU_MALO, N_TAPS_LMS)
E_MALO_200 = float(np.nanmean(np.abs(E_MALO[:200])))     # ~253
E_BUENO_200 = float(np.mean(np.abs(E_LMS[MU_DEMO][:200])))
