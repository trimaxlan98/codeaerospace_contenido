# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 6.2 Remedios".
...
M = S.MEDIDO
LOOKUP_501 = M["c501_lookup"]                                    # 597 (motor)
SCAN_1 = M["c1_scan"]                                            # 22,363 (motor)
OPTFOR_501, OPTFOR_1 = M["sniff_501_luego_1"]                    # 597, 459,555 (motor)
IDX_501, IDX_1 = M["remedio_indice"]                             # 6, 1,974 (motor)
PSP_VARIANTES = M["psp_variantes"]                               # 0 (motor)
RAZON_INDICE = S.razon(OPTFOR_1, IDX_1)                          # ~233x (calculado)
...
def caja_cliente(etiqueta, color=None, ancho=1.5, alto=0.85, fs=26):
    """Caja que representa UN cliente concreto (501 o el mayorista): una
    sola palabra o numero adentro (ASCII). Se usa igual en los 4 clips
    para que el espectador reconozca siempre al mismo personaje."""
    ...
    t = Text(str(etiqueta), font=FUENTE_HUD, font_size=fs, color=C_TITULO)
...
def barra_compilar(alto=1.3, ancho=0.7, color=None):
    """Barra de 'compilar': SIN cifra (no hay medicion de CPU de compilar
    en la libreria); solo representa que la ejecucion paga este costo."""
    return Rectangle(width=ancho, height=alto, stroke_width=0,
                     fill_color=C_TENUE if color is None else color,
                     fill_opacity=0.85)
