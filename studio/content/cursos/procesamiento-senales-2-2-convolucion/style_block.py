# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 2.2 Convolucion: deslizar y sumar".
...
# --- Numeros de la leccion --------------------------------------------
X_CORTO = np.array([0.8, -0.4, 0.9, 0.3, -0.7, 0.5, 0.2, -0.6])
H_CORTO = np.array([0.5, 0.35, -0.2, 0.15])
Y_CORTO = convolucion(X_CORTO, H_CORTO)
PASOS = pasos_convolucion(X_CORTO, H_CORTO)
N_X, M_H = len(X_CORTO), len(H_CORTO)              # 8 y 4
N_SALIDA = len(Y_CORTO)                            # 11 = N + M - 1
MACS_CORTO = macs_convolucion(X_CORTO, H_CORTO)    # 32
N_LLENOS = N_X - M_H + 1                           # 5 salidas "llenas"

_RNG = np.random.default_rng(11)
X_LARGO = np.round(_RNG.normal(0.0, 0.5, 24), 3)
H_LARGO = np.array([0.4, 0.3, -0.2, 0.15, 0.1, -0.05, 0.05, 0.02, 0.01])
Y_LARGO = convolucion(X_LARGO, H_LARGO)
N_LARGO, M_LARGO = len(X_LARGO), len(H_LARGO)      # 24 y 9
SALIDA_LARGO = len(Y_LARGO)                        # 32
MACS_LARGO = macs_convolucion(X_LARGO, H_LARGO)    # 216 = 24 x 9

A_ESTABLE, A_INESTABLE = 0.85, 1.06
N_BIBO = 60
H_ESTABLE = respuesta_impulso([1.0], [1.0, -A_ESTABLE], N_BIBO)
H_INESTABLE = respuesta_impulso([1.0], [1.0, -A_INESTABLE], N_BIBO)
SUMA_ESTABLE = suma_abs(H_ESTABLE)                 # 6.66
SUMA_INESTABLE = suma_abs(H_INESTABLE)             # crece sin freno
ULTIMO_INESTABLE = float(H_INESTABLE[-1])
