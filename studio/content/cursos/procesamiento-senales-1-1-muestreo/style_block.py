# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 1.1 Muestrear: el espectro
# se repite". Bloque de estilo del proyecto: se antepone al script de
...
# discrepar.
FS = 800.0                     # Hz: el muestreo que SI cumple
N_MUESTRAS = 64
T_MUESTRA_MS = 1000.0 / FS     # 1.25 ms
TK, XK = muestras_de(vibracion, FS, N_MUESTRAS)
T_VENTANA = N_MUESTRAS / FS    # 80 ms de señal

F_BASE, DB_BASE = espectro_analogico(vibracion, FS, banda=380.0, n=4096)
BANDA = banda_ocupada(F_BASE, DB_BASE, -40.0)        # 222.7 Hz medidos
GUARDA_OK = guarda(FS, BANDA)                        # 354.7 Hz
F_REP, DB_REP = replicas(F_BASE, DB_BASE, FS, k=2)

FS_MALA = 300.0                # Hz: por debajo de 2 x 222.7
GUARDA_MALA = guarda(FS_MALA, BANDA)                 # -145.3 Hz
SOLAPE_MALO = solape_db(F_BASE, DB_BASE, FS_MALA, BANDA)   # -12.1 dB
F_REP_MALA, DB_REP_MALA = replicas(F_BASE, DB_BASE, FS_MALA, k=2)
F_ALTA = 220.0                 # el modo estructural rapido
F_ALIAS = alias_de(F_ALTA, FS_MALA)                  # 80.0 Hz

T_DENSO = np.linspace(0.0, 0.06, 1500)
TK_MALA, XK_MALA = muestras_de(vibracion, FS_MALA, int(0.08 * FS_MALA))
ERR_MALA = error_rms(vibracion(T_DENSO),
                     reconstruir_sinc(TK_MALA, XK_MALA, T_DENSO))   # 0.162
ERR_BIEN = error_rms(vibracion(T_DENSO),
                     reconstruir_sinc(TK, XK, T_DENSO))             # 0.004

FC_AA, ORDEN_AA = 120.0, 6     # el antialias que salva el muestreo lento
DB_AA = con_antialias(F_BASE, DB_BASE, FC_AA, ORDEN_AA)
ATEN_AA = butter_db(F_ALTA, FC_AA, ORDEN_AA)         # -31.6 dB en 220 Hz
SOLAPE_AA = solape_db(F_BASE, DB_AA, FS_MALA, BANDA)  # -43.4 dB
F_REP_AA, DB_REP_AA = replicas(F_BASE, DB_AA, FS_MALA, k=2)
