# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 3.1 El orden de las llaves".
...
PAG_PED = M["paginas_pedidos"]                                  # 22,183 (motor)
FPP = S.filas_por_pagina(FILAS_PED, PAG_PED)                    # 67.6
C501 = S.filas_cliente(501)                                     # 191
C1 = S.filas_cliente(1)                                         # 149,970
PROF = S.profundidad(FILAS_PED, FPP, S.fanout_int(4))           # 3

# --- 3.1 El orden de las llaves -----------------------------------------
FRAC_ENTREGADO = S.fraccion_estatus()["entregado"]              # 0.91545...
FILAS_2025 = S.filas_rango("2025-01-01", "2026-01-01")          # 418,481
FILAS_2025_CANC = S.filas_rango("2025-01-01", "2026-01-01",
                                "cancelado")                     # 33,275
LEC_FE = M["canc2025_fecha_estatus"]                             # 1,978
LEC_EF = M["canc2025_estatus_fecha"]                             # 161
RAZON_LEC = S.razon(LEC_FE, LEC_EF)                              # ~12.3x
...
# Tonos de UN matiz (azul, el color de un indice) para los cuatro estatus:
# nunca verde/ambar/cian/violeta como color de CATEGORIA. El rojo se
# reserva para resaltar el estatus que se busca en el clip. Son colores
# (claro a oscuro), no opacidad: a este tamano la opacidad sola no se
# distingue sobre el fondo casi negro. Los cuatro tonos separados al
# maximo (casi blanco a casi negro) para que se lean a 1080p; ademas,
# "cancelado" lleva un borde claro mas grueso (patron, no solo color).
TONOS_ESTATUS = {"pendiente": "#dbeafe", "enviado": "#93c5fd",
                 "entregado": "#2563eb", "cancelado": "#1e3a8a"}


def fila_indice(estatus, ancho=1.5, alto=0.18, resaltar=False):
    """Una fila de una lista ordenada (un renglon de indice): un
    rectangulo. `resaltar=True` la pone en rojo (el caso que se busca);
    si no, en su tono de azul segun `TONOS_ESTATUS`. "cancelado" ademas
    lleva un borde claro mas grueso: un patron, no solo un tono, para
    que se distinga aunque el tono no se lea bien."""
    if resaltar:
        return Rectangle(width=ancho, height=alto, stroke_width=2.0,
                         stroke_color=C_MALO, fill_color=C_MALO,
                         fill_opacity=0.92)
    color = TONOS_ESTATUS[estatus]
    borde = C_TITULO if estatus == "cancelado" else color
    grosor = 2.2 if estatus == "cancelado" else 1.0
    return Rectangle(width=ancho, height=alto, stroke_width=grosor,
                     stroke_color=borde, fill_color=color, fill_opacity=0.9)


def lista_indice(grupos, resaltado=None, ancho=1.5, alto=0.18, buff=0.035,
                 sep_grupo=0.12):
    """Columna vertical de filas de indice. `grupos`: lista de listas de
    estatus (cada sublista, un bloque separado por un hueco visible).
    `resaltado`: el estatus que se pinta en rojo. Devuelve (columna,
    bloques) para poder enmarcar o medir un bloque en particular."""
    bloques = []
    col = VGroup()
    for grupo in grupos:
        b = VGroup(*[fila_indice(e, ancho, alto, resaltar=(e == resaltado))
                     for e in grupo])
        b.arrange(DOWN, buff=buff)
        bloques.append(b)
        col.add(b)
    col.arrange(DOWN, buff=sep_grupo)
    return col, bloques


def leyenda_estatus(ancho=0.44, alto=0.28):
    """Fila horizontal con las cuatro muestras de color y su nombre."""
    g = VGroup()
    for e in S.ESTATUS:
        sw = fila_indice(e, ancho=ancho, alto=alto)
        et = tag_junto(sw, e, RIGHT, buff=0.12, font_size=21)
        g.add(VGroup(sw, et))
    g.arrange(RIGHT, buff=0.55)
    return g


def corchete(mobjeto, lado="izquierda", color=None, buff=0.1, tick=0.12):
    """Corchete vertical que abraza `mobjeto` por un lado: marca el rango
    que un indice tiene que recorrer sobre su lista."""
    color = C_CALCULO if color is None else color
    borde = mobjeto.get_left() if lado == "izquierda" else mobjeto.get_right()
    signo = 1 if lado == "izquierda" else -1
    x = borde[0] - signo * buff
    y0, y1 = mobjeto.get_bottom()[1], mobjeto.get_top()[1]
    tronco = Line([x, y0, 0], [x, y1, 0], stroke_color=color, stroke_width=3)
    r_sup = Line([x, y1, 0], [x + signo * tick, y1, 0], stroke_color=color,
                stroke_width=3)
    r_inf = Line([x, y0, 0], [x + signo * tick, y0, 0], stroke_color=color,
                stroke_width=3)
    return VGroup(tronco, r_sup, r_inf)
