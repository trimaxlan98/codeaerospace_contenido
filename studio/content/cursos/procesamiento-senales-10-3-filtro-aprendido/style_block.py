# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 10.3 El filtro aprendido".
...
# --- Numeros de la leccion --------------------------------------------
# Una red lineal aprendiendo a filtrar por ejemplos, y lo que descubre.
N_TAPS_RED = 17
PASOS = 400
N_EJEMPLOS = 3000            # los pares (entrada, salida) del entrenamiento
W_APRENDIDO, HIST, H_OBJETIVO = entrenar_filtro(N_EJEMPLOS, N_TAPS_RED,
                                                PASOS, 0.05, 9)
COS, ERR_REL = parecido(W_APRENDIDO, H_OBJETIVO)   # 1.000000 y 1.9e-09
ERR_INICIAL = float(HIST[0])                # 0.1979
ERR_FINAL = float(HIST[-1])                 # 7.5e-19
W_RESP = respuesta_frec(W_APRENDIDO, [1.0], 1024)
H_RESP = respuesta_frec(H_OBJETIVO, [1.0], 1024)
DIF_RESP = float(np.abs(W_RESP[1] - H_RESP[1]).max())

# donde una red SI aporta: deshacer una no linealidad
ERR_LINEAL, ERR_RED, HIST_RED = red_no_lineal()
MEJORA_RED = 20.0 * math.log10(ERR_LINEAL / ERR_RED)   # 5.2 dB
N_SAT = 2000
_K = np.arange(N_SAT)
LIMPIA_SAT = np.sin(2 * np.pi * 0.013 * _K) * 0.9
SUCIA_SAT = np.tanh(2.2 * LIMPIA_SAT)       # el amplificador saturado
GANANCIA_LIN = float(np.dot(SUCIA_SAT, LIMPIA_SAT)
                     / np.dot(SUCIA_SAT, SUCIA_SAT))
MEJOR_LINEAL = GANANCIA_LIN * SUCIA_SAT
