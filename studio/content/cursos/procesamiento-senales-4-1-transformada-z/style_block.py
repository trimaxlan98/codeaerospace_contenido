# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 4.1 La transformada Z".
...
# --- Numeros de la leccion --------------------------------------------
# z^-1 es "una muestra de retraso": con eso la ecuacion en diferencias se
# vuelve un polinomio, y sus raices son el filtro.
B_FIR = np.array([1.0, -1.6, 0.9])           # dos ceros complejos
CEROS_FIR, _, K_FIR = zpk(B_FIR)
R_CERO = float(np.abs(CEROS_FIR[0]))                     # 0.949
ANG_CERO = float(np.degrees(np.angle(CEROS_FIR[0])))     # 32.5 grados
W_CERO = float(np.angle(CEROS_FIR[0]))
W_FIR, MAG_FIR, FASE_FIR = respuesta_frec(B_FIR)
H_FIR = respuesta_impulso(B_FIR, [1.0], 12)

W0_RES = np.pi / 4                            # donde se pone el polo
RADIOS = (0.70, 0.92, 1.05)
RES = {r: resonador(r, W0_RES) for r in RADIOS}
H_RES = {r: respuesta_impulso(*RES[r], 80) for r in RADIOS}
# OJO: h[79] vale 0 para los tres (cae en un cruce por cero del seno);
# lo que distingue estable de inestable es el MAXIMO de la cola.
COLA = {r: float(np.abs(H_RES[r][-10:]).max()) for r in RADIOS}  # 0 / 0 / 60.6
SUMA_H_RES = {r: suma_abs(H_RES[r]) for r in RADIOS}    # 3.3 / 11.1 / 808.4
MAX_H = {r: float(np.abs(H_RES[r]).max()) for r in RADIOS}
ESTABLE = {r: es_estable(RES[r][1]) for r in RADIOS}
W_RES, MAG_RES, _ = respuesta_frec(*RES[0.92], n=1024)
PICO_RES = float(MAG_RES.max())               # 19.3 dB
