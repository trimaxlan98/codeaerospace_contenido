# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 5.1 Lo que el optimizador sabe".
...
# El histograma es un MODELO del algoritmo de SQL Server (el real es
# propietario): solo el EQ_ROWS del cliente 1 coincide con lo medido.
HISTO = S.histograma()                                           # 181 pasos
N_PASOS = len(HISTO)                                             # 181 (cian)
PASOS_MAX = S.PASOS_HISTOGRAMA                                   # 200 (gris, dato)
IDX_C1 = 0                                                        # paso propio del cliente 1 (hi=1)
IDX_EJEMPLO = 140                                                 # paso "normal" para nombrar RANGE_HI_KEY/EQ_ROWS
PASO_C1 = S.paso_de(HISTO, 1)
C1_MODELO = PASO_C1["eq"]                                         # 149,970 (cian, del histograma)
EQ_C1_MOTOR = M["eq_rows_c1"]                                     # 149,970 (ambar, medido)

CLIENTES_DIST = S.clientes_distintos()                            # 189,248 (cian)
EST_VAR = S.estimado_variable()                                   # 7.93 (cian)

UMBRAL_NUEVO = int(S.umbral_estadisticas(FILAS_PED))              # 38,729 (cian)
UMBRAL_VIEJO = int(S.umbral_estadisticas_viejo(FILAS_PED))        # 300,500 (cian)
...
def barras_histograma(histo, ancho=10.4, alto_pared=1.7, alto_recorte=3.1,
                      base_y=-2.1, color=None, indice_recortado=None):
    """VGroup de barras (una por PASO de `histo`), altura LINEAL segun
    eq+range: la "pared" de pasos normales llega hasta `alto_pared` (el
    mas alto de ellos la toca); si `indice_recortado` no es None, ESA
    barra (la torre del mayorista) rompe la pared y se RECORTA en
    `alto_recorte` (mas alta que la pared) con una marca de quiebre y un
    tallo que sigue de largo: su valor real se guarda para el clip
    siguiente, aqui NO se rotula con cifra. Devuelve (barras, quiebre)."""
    color = C_CREE if color is None else color
    n = len(histo)
    alturas = [p["eq"] + p["range"] for p in histo]
    normales = [h for i, h in enumerate(alturas) if i != indice_recortado]
    techo = max(normales) if normales else max(alturas)
    escala = alto_pared / techo
    paso_x = ancho / n
    barras, quiebre = VGroup(), VGroup()
    for i, h in enumerate(alturas):
        recortada = (i == indice_recortado)
        alto_i = alto_recorte if recortada else max(h * escala, 0.05)
        r = Rectangle(width=paso_x * 0.8, height=alto_i, stroke_width=0,
                     fill_color=color, fill_opacity=0.95 if recortada else 0.75)
        cx = -ancho / 2 + paso_x * (i + 0.5)
        r.move_to([cx, base_y + alto_i / 2, 0])
        barras.add(r)
        if recortada:
            top = base_y + alto_i
            zig = VMobject(stroke_color=color, stroke_width=3)
            zig.set_points_as_corners([
                [cx - paso_x * 0.4, top, 0], [cx - paso_x * 0.12, top + 0.09, 0],
                [cx + paso_x * 0.12, top - 0.09, 0], [cx + paso_x * 0.4, top, 0]])
            tallo = Arrow([cx, top + 0.05, 0], [cx, top + 0.55, 0], buff=0,
                         stroke_width=3, color=color,
                         max_tip_length_to_length_ratio=0.5)
            quiebre.add(zig, tallo)
    return barras, quiebre
