# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 10.1 La señal analitica".
...
F_PORTADORA = 60.0
F_MOD = 3.0
ENV_REAL = 1.0 + 0.5 * np.sin(2 * np.pi * F_MOD * T_A)
X_A = ENV_REAL * np.sin(2 * np.pi * F_PORTADORA * T_A)

X_ANALITICA = analitica(X_A)
ENV_MEDIDA = envolvente(X_A)
FASE_A = fase_inst(X_A)
FREC_A = frecuencia_inst(X_A, FS_A)
INTERIOR = slice(50, -50)
ERR_ENV = float(np.sqrt(np.mean((ENV_MEDIDA[INTERIOR]
                                 - ENV_REAL[INTERIOR]) ** 2)))     # 0.0000
FREC_MEDIA = float(np.mean(FREC_A[INTERIOR]))                      # 60.00

# la analitica de un tono puro es un fasor de modulo constante
X_TONO = np.sin(2 * np.pi * F_PORTADORA * T_A)
XA_TONO = analitica(X_TONO)
MOD_TONO = float(np.mean(np.abs(XA_TONO[INTERIOR])))               # 1.0000
DESV_MOD = float(np.std(np.abs(XA_TONO[INTERIOR])))
RE_TONO = np.real(XA_TONO)
IM_TONO = np.imag(XA_TONO)
