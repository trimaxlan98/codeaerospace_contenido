# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 6.2 Biquads en cascada".
...
ATEN_PEDIDA = 45.0
ORDEN = 10
B, A = iir_cheby1(ORDEN, 0.5, 0.15, FS_D)
N_BIQUADS, SOS = secciones(B, A)               # 5 secciones
W, MAG, _ = respuesta_frec(B, A, 2048)
POLOS_EXACTOS = np.roots(A)
RADIO_EXACTO = float(np.abs(POLOS_EXACTOS).max())      # 0.9874

BITS = (16, 12)
CUANT = {b: polos_cuantizados(B, A, b) for b in BITS}
ERR_DIRECTA = {b: CUANT[b][3] for b in BITS}   # 2.09e-01 / 4.26e-01
ERR_CASCADA = {b: CUANT[b][4] for b in BITS}   # 1.78e-04 / 1.94e-03
RAZON = {b: ERR_DIRECTA[b] / ERR_CASCADA[b] for b in BITS}   # 1173x / 220x
POLOS_DIRECTA = {b: CUANT[b][1] for b in BITS}
POLOS_CASCADA = {b: CUANT[b][2] for b in BITS}
RADIO_DIRECTA = {b: float(np.abs(POLOS_DIRECTA[b]).max()) for b in BITS}
# 1.1121 a 16 bits y 1.2430 a 12: los polos SE SALEN del circulo. El
# filtro en forma directa no se degrada, se vuelve INESTABLE.
ESTABLE_DIRECTA = {b: bool(RADIO_DIRECTA[b] < 1.0) for b in BITS}
A_CUANT = {b: np.round(A / (2.0 ** -(b - 1))) * (2.0 ** -(b - 1))
           for b in BITS}
MAG_DIRECTA = {}
for _b in BITS:
    MAG_DIRECTA[_b] = respuesta_frec(B, A_CUANT[_b], 2048)[1]
PICO_EXACTO = float(MAG.max())                          # -0.00 dB
PICO_DIRECTA = {b: float(MAG_DIRECTA[b].max()) for b in BITS}  # -5.99 / inf
# A 16 bits cruzan el circulo SEIS de los diez polos (radios 1.0032,
# 1.0673 x2 y 1.1121 x2, mas uno); a 12 bits, cinco.
