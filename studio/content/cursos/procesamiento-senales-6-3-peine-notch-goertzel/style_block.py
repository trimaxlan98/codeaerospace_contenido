# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 6.3 Peine, notch y Goertzel".
...
M_PEINE = 8
G_PEINE = 0.85
B_PEINE, A_PEINE = peine(M_PEINE, G_PEINE)
W_PEINE, MAG_PEINE, _ = respuesta_frec(B_PEINE, A_PEINE, 2048)
DIENTES = [float(W_PEINE[i] / np.pi) for i in range(1, len(MAG_PEINE) - 1)
           if MAG_PEINE[i] > MAG_PEINE[i - 1]
           and MAG_PEINE[i] >= MAG_PEINE[i + 1]]      # 0.25 / 0.50 / 0.75
MAX_PEINE = float(MAG_PEINE.max())                    # +5.34 dB
MIN_PEINE = float(MAG_PEINE.min())                    # -16.48 dB
H_PEINE = respuesta_impulso(B_PEINE, A_PEINE, 20)

W0_RES = 0.15 * np.pi
R_RES = 0.95
B_RES, A_RES = resonador(R_RES, W0_RES)
W_RES, MAG_RES, _ = respuesta_frec(B_RES, A_RES, 2048)
PICO_RES = float(MAG_RES.max())
W_PICO_RES = float(W_RES[int(np.argmax(MAG_RES))] / np.pi)   # 0.150

W0_NOTCH = 0.4 * np.pi
R_NOTCH = 0.97
B_NOTCH, A_NOTCH = notch(R_NOTCH, W0_NOTCH)
W_N, MAG_N, _ = respuesta_frec(B_NOTCH, A_NOTCH, 4096)
# OJO: el cero esta EXACTAMENTE sobre el circulo, asi que el minimo real
# es -infinito y lo que se mida depende de lo fina que sea la malla
# (-39.7 dB con 2048 puntos, -240 dB con 4096). NO se rotula el minimo.
# Lo honesto es el ANCHO del agujero y lo poco que toca al vecino.
ANCHO_20DB = float(np.sum(MAG_N < -20.0) / len(MAG_N))
W_VECINO = 0.35 * np.pi
H_VECINO = float(MAG_N[int(np.argmin(np.abs(W_N - W_VECINO)))])   # 0.10 dB

N_G = 128
K_G = 17
X_G = (np.cos(2 * np.pi * K_G * np.arange(N_G) / N_G)
       + 0.3 * np.cos(2 * np.pi * 40 * np.arange(N_G) / N_G))
POR_GOERTZEL, POR_DFT = goertzel(X_G, K_G, N_G)        # 64.0 y 64.0
DIF_G = abs(POR_GOERTZEL - POR_DFT)                    # 0.0 exacto
MACS_G, MACS_FFT = macs_goertzel(N_G)                  # 128 y 448
