# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 2.3 El indice que cubre".
...
HOJAS_C1 = M["c1_cubriente"] - PROF                              # 782 hojas
FILAS_POR_HOJA_C1 = C1 / HOJAS_C1                                # ~191.8
...
def celda_campo(nombre, color=C_INDICE, alto=0.85, fs=22, relleno=0.14,
                opacidad_borde=1.0):
    """Una celda de una hoja de indice: caja con el nombre de la columna
    adentro (ASCII, una sola palabra). El ancho se ajusta al texto. Letra
    >= 20 px (a 1080p, menos no se lee)."""
    etq = Text(nombre, font_size=fs, color=C_TITULO)
    ancho = etq.width + 0.4
    caja = RoundedRectangle(width=ancho, height=alto, corner_radius=0.08,
                            stroke_color=color, stroke_width=2.0,
                            stroke_opacity=opacidad_borde,
...
def fila_de(celdas, buff=0.08):
    """Arregla una lista de celdas (`celda_campo`) en una fila horizontal:
    la hoja del indice, de izquierda a derecha."""
    fila = VGroup(*celdas)
