# =====================================================================
# CO.DE Academy - "Tesis 6G · 2.3 Consenso con traidores". Bloque de estilo
...
LAMBDA = np.degrees(np.arccos(R_T * np.cos(np.radians(EL_MIN))
                              / (R_T + H_KM))) - EL_MIN
Q4, Q6, Q7 = (ntn.quorum_pbft(n) for n in (4, 6, 7))
NS = [10, 30, 100, 300, 1000]
GRUPOS = [T6.mensajes_por_grupos(n, 10) for n in NS]
G1000 = GRUPOS[-1]
# reputacion: cinco candidatos, uno de ellos traidor que propone decisiones invalidas
W_REP = (1.0, 1.0, 0.5)                              # pesos ELEGIDOS (la tesis no los fija)
_rng = np.random.default_rng(42)
UPTIME = np.round(_rng.uniform(0.85, 0.99, 5), 2)
CALIDAD = np.round(_rng.uniform(0.55, 0.8, 5), 2)
TRAIDOR = int(np.argmax(UPTIME + CALIDAD))           # el MEJOR de partida es el traidor
FALLOS = np.zeros((5, 5))                            # rondas x candidatos
for _r in range(1, 5):
    FALLOS[_r] = FALLOS[_r - 1]
    FALLOS[_r, TRAIDOR] += 1                          # propone algo invalido cada ronda
REPS = np.array([T6.reputacion(UPTIME, CALIDAD, FALLOS[r], W_REP) for r in range(5)])
LIDER = REPS.argmax(axis=1)
RONDA_CAMBIO = int(np.argmax(LIDER != TRAIDOR))
