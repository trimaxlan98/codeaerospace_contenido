# =====================================================================
# CO.DE Academy - "Comunicaciones digitales · 1.1 El mensaje digital:
# muestrear y cuantizar". Bloque de estilo del proyecto. Se antepone al
# script de CADA clip; los clips NO repiten imports: solo definen su
# ClipN(Scene).
#
# Este bloque es el MOLDE de la familia "Comunicaciones digitales": las
# 17 lecciones restantes copian este archivo y solo cambian la cabecera
# y la tabla de numeros de su leccion. Por eso la paleta, los rotulos y
...
# discrepar.
F_LENTA, F_RAPIDA = 1.3, 3.1     # Hz de la telemetria (dos tonos)
T_FIN = 2.0                      # segundos de señal en pantalla


def SENAL_TELE(t):
    """La telemetria de la sonda (temperatura normalizada, [-1, 1])."""
    return (0.62 * math.sin(2 * math.pi * F_LENTA * t)
            + 0.30 * math.sin(2 * math.pi * F_RAPIDA * t + 0.7))


T_CURVA = np.linspace(0.0, T_FIN, 480)
Y_CURVA = np.array([SENAL_TELE(t) for t in T_CURVA])
FS_BUENA = 10.0                  # Hz > 2 x 3.1: reconstruye
F_SENO_ALIAS = 7.0               # el seno que se disfraza...
FS_ALIAS = 10.0                  # ...muestreado a 10 Hz...
F_ALIAS = alias_de(F_SENO_ALIAS, FS_ALIAS)   # ...aparenta 3 Hz
BITS_GRUESO, BITS_FINO = 3, 8
SNR_GRUESO = snr_cuantizacion(Y_CURVA, BITS_GRUESO)   # ~19.6 dB
SNR_FINO = snr_cuantizacion(Y_CURVA, BITS_FINO)       # ~49.7 dB
T_MUESTRA_PCM = 0.35             # la muestra que se vuelve palabra
Y_MUESTRA_PCM = SENAL_TELE(T_MUESTRA_PCM)
BITS_PCM = bits_de_muestra(Y_MUESTRA_PCM, BITS_FINO)  # 8 bits
