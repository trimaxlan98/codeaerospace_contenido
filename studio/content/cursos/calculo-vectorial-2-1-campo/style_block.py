# =====================================================================
# CO.DE Academy - "Cálculo vectorial · 2.1 El campo vectorial: flechas por todas partes".
...
# Todo valor que se rotule sale de aqui o de la libreria, nunca escrito
# a mano en el clip: lo dibujado y la cifra escrita no pueden discrepar.
PAISAJE = paisaje_colinas                # el paisaje de la familia (1.1)
Z_MAX = 2.2                              # tope de altura (para color_calor)
NIVELES = [0.25, 0.55, 0.9, 1.3, 1.7, 2.05]      # las mismas curvas de 1.1

CAMPO_VIENTO = campo_viento              # clip 1: el campo que se lee
# clip 1: tres puntos donde se lee F(x, y) a mano, en tres cuadrantes
# distintos (fuera de la esquina del panel, que vive arriba a la derecha)
PUNTOS_LECTURA = [(-2.8, -1.5), (-0.5, 2.2), (2.6, -2.0)]

# clip 3: el campo gradiente del paisaje 1.1; un punto de ladera empinada
# (cerca de la colina alta) y uno casi llano (esquina lejana del mapa)
P_EMPINADO = (2.5, 0.5)
P_LLANO = (3.5, -2.9)

# clip 4: viento sobre un perfil (cizalla + remolino) y gravedad de un
# planeta (radial hacia el centro); el punto cercano al planeta muestra
# la flecha grande
P_CERCA_PLANETA = (0.55, 0.25)
