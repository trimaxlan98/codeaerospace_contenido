# =====================================================================
# CO.DE Academy - "SDR · 6.1 ADS-B: aviones".
...
BITS = S.adsb_identificacion()                    # avion FICTICIO: 0D0C38 CODE101
MAG = S.adsb_captura(BITS, semilla=1)              # |IQ| a 2 MS/s, 12 dB
OFF, DEC, RECHAZADOS = S.adsb_buscar(MAG)          # 137, dict, candidatos falsos
_, CORR = S.adsb_buscar_preambulo(MAG)
KLM = S.adsb_decodificar(S.adsb_captura(np.array(
    [int(c) for c in bin(int("8D4840D6202CC371C32CE0576098", 16))[2:].zfill(112)]),
    snr_db=40), 137)                               # mensaje REAL: 4840D6 KLM1023
