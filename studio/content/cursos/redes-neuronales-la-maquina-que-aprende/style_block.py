# =====================================================================
# CO.DE Academy - "Redes neuronales: la maquina que aprende"
...
from aprendizaje import (camino_descenso, curva_perdida, datos_circulos,
                         datos_dos_nubes, datos_recta, datos_xor,
                         descenso_1d, entrenar_mlp, entrenar_regresion,
                         frontera_imagen, puntos_datos, recta_modelo,
                         retropropagacion)
...
# --- Paleta del curso -------------------------------------------------
# Regla: los DATOS son cian/violeta, el MODELO es ambar, el ERROR es rojo,
# el ACIERTO es verde. No mezclar roles.
C_TITULO = CODE_INK          # #e8edf3 titulos
C_TENUE = CODE_MUTED         # #94a0b0 pies y elementos secundarios
C_ACENTO = CODE_ACCENT       # #f59e0b ambar: el modelo (rectas, fronteras)
...
C_PERDIDA = "#f43f5e"        # rojo: error, perdida, divergencia
C_OK = "#34d399"             # verde: acierto, modelo que generaliza
C_EJE = "#31414f"            # gris azulado de ejes y rejilla
...
def tag_eje(ejes, texto, x, y, direccion=DOWN, buff=0.16, font_size=18):
    """Nombre del eje (mobiliario de la figura, no rotulo narrativo)."""
    t = Text(texto, font_size=font_size, color=C_TENUE)
    t.set_opacity(0.75)
    t.next_to(ejes.c2p(x, y), direccion, buff=buff)
    return t
...
# --- Ejes -------------------------------------------------------------
def ejes_plano(lim=2.2, lado=4.6):
    """Plano de datos cuadrado [-lim, lim]^2, sin ticks (divulgacion)."""
    return Axes(
        x_range=[-lim, lim, lim],
        y_range=[-lim, lim, lim],
        x_length=lado,
        y_length=lado,
        tips=False,
        axis_config={"stroke_width": 2.5, "color": C_EJE,
                     "include_ticks": False},
    )


def ejes_curva(x_max=2.0, y_max=2.0, x_min=None, y_min=0.0, x_length=8.8,
               y_length=4.2):
    """Ejes para curvas de perdida / paisajes de error, sin ticks."""
    x0 = -x_max if x_min is None else x_min
    return Axes(
        x_range=[x0, x_max, x_max],
        y_range=[y_min, y_max, y_max],
        x_length=x_length,
        y_length=y_length,
        tips=False,
        axis_config={"stroke_width": 2.5, "color": C_EJE,
