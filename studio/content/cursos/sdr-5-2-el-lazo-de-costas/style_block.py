# =====================================================================
# CO.DE Academy - "SDR · 5.2 El lazo de Costas".
...
C_EJE = S.C_EJE              # mobiliario: ejes y marcos
bloque, conectar, flujo = S.bloque, S.conectar, S.flujo
...
SIM, RX, FASE_REAL = S.escenario_costas()       # BPSK, 0.002 c/s, 1.1 rad, 10 dB
ANCHOS = (0.005, 0.02, 0.08)                       # Bn*T (parametros)
LAZOS = {}
for _bn in ANCHOS:
    _out, _fase, _err = S.costas_bpsk(RX, _bn)
    _te = S.tiempo_enganche(_fase, FASE_REAL)
    LAZOS[_bn] = dict(out=_out, fase=_fase, err=_err, enganche=_te,
                      jitter=np.degrees(S.jitter_fase(_fase, FASE_REAL, 1000)))
# 8 semillas: enganche = MEDIANA (918 / 113 / 30 simbolos); temblor =
# MEDIA (1.4 / 2.5 / 5.2 grados). Son las que van a pantalla.
ENG_MED, JIT_MED = {}, {}
for _bn in ANCHOS:
    _t, _j = [], []
    for _s in range(1, 9):
        _si, _rx, _fr = S.escenario_costas(semilla=_s)
        _o, _f, _e = S.costas_bpsk(_rx, _bn)
        _t.append(S.tiempo_enganche(_f, _fr) or 10 ** 6)
        _j.append(np.degrees(S.jitter_fase(_f, _fr, 1000)))
    ENG_MED[_bn], JIT_MED[_bn] = int(np.median(_t)), float(np.mean(_j))
SIM_I, RX_I, FR_I = S.escenario_costas(fase0=2.3, df_rs=0.0, semilla=3)
OUT_I, _, _ = S.costas_bpsk(RX_I, 0.02)
INVERTIDO = S.ambiguedad_180(SIM_I, OUT_I)         # True: engancha al reves
