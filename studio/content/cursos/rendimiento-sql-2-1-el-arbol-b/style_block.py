# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 2.1 El arbol B".
...
FAN = S.fanout_int(4)                                            # 622
PROF = S.profundidad(FILAS_PED, FPP, FAN)                        # 3
...
def capacidad_niveles(niveles, fpp=FPP, fanout=FAN):
    """Filas maximas que caben en un arbol de `niveles` niveles: se
    RECORRE la funcion S.profundidad con busqueda binaria (no se despeja
    a mano). Sirve para el rotulo 'cuantas filas caben con un nivel mas'
    del clip 3 (capacidad_niveles(PROF + 1))."""
    hi = 1
    while S.profundidad(hi, fpp, fanout) <= niveles:
        hi *= 10
    lo, hi_b = 1, hi
    while lo < hi_b:
        mid = (lo + hi_b + 1) // 2
        if S.profundidad(mid, fpp, fanout) <= niveles:
            lo = mid
        else:
            hi_b = mid - 1
    return lo
...
def umbral_niveles(niveles, fpp=FPP, fanout=FAN, exp_ini=1.0, exp_fin=12.0):
    """Exponente (log10 de filas) donde `S.profundidad` pasa a valer MAS de
    `niveles`: frontera real (no entera) para dibujar el escalon, hallada
    recorriendo la funcion con bisección sobre el exponente."""
    lo, hi = exp_ini, exp_fin
    for _ in range(60):
        mid = (lo + hi) / 2
        if S.profundidad(10 ** mid, fpp, fanout) <= niveles:
            lo = mid
        else:
            hi = mid
    return hi
