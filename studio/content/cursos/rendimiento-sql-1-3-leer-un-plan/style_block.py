# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 1.3 Leer un plan".
...
M = S.MEDIDO
FILAS_PED = M["filas_pedidos"]                                  # 1,500,000 (cian: conteo)
PAG_PED = M["paginas_pedidos"]                                  # 22,183 (motor)
FPP = S.filas_por_pagina(FILAS_PED, PAG_PED)                    # 67.6
C501 = S.filas_cliente(501)                                     # 191 (cian: conteo)
C1 = S.filas_cliente(1)                                         # 149,970
PROF = S.profundidad(FILAS_PED, FPP, S.fanout_int(4))           # 3
SCAN_PED = M["scan_pedidos"]                                    # 22,363 (ambar: motor)
PK_SEEK = M["pk_seek"]                                          # 3 (ambar: motor)
C501_LOOKUP = M["c501_lookup"]                                  # 597 (ambar: motor)
HILOS_TOTAL = M["scan_count_paralelo"]                          # 9 (ambar: motor)
HILOS = HILOS_TOTAL - 1                                         # 8 (cian: aritmetica)
...
def paleta_categorica(n, a=None, b=None):
    """`n` tonos de UN solo matiz (azul indice -> gris tenue por defecto):
    para distinguir cosas iguales (hilos, cubetas de un hash) sin pisar
    los colores de papel (rojo/verde/ambar/violeta ya significan algo)."""
    ca = ManimColor(C_INDICE if a is None else a)
    cb = ManimColor(C_TENUE if b is None else b)
    if n <= 1:
        return [ca]
    return [interpolate_color(ca, cb, i / (n - 1)) for i in range(n)]


def viaje_de_vuelta(origen, destino, color, ancho=1.6):
    """Dos arcos entre `origen` y `destino`: la ida y la vuelta de un key
    lookup (baja al indice, sube a la pagina de datos y regresa). Se usa
    para distinguir un Seek (solo baja) de un Lookup (baja y vuelve)."""
    ida = ArcBetweenPoints(origen, destino, angle=-PI / 2.6,
                          stroke_color=color, stroke_width=ancho)
    vuelta = ArcBetweenPoints(destino, origen, angle=-PI / 2.6,
                             stroke_color=color, stroke_width=ancho)
    return VGroup(ida, vuelta)
