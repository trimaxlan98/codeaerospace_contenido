# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 5.2 Adivinar a ciegas".
...
ESTIMADO = S.estimado_variable()                                # 7.93
ERROR = S.razon(C1, ESTIMADO)                                   # ~18,919x
ERROR_REDONDO = int(round(ERROR / 100.0)) * 100                 # ~18,900
LIT = M["literal_c1"]                                           # 22,363 (motor)
VARLOC = M["variable_local"]                                    # 450,173 (motor)
RAZON4 = S.razon(VARLOC, LIT)                                   # ~20x
...
def torre_histograma(n=9, indice_torre=6, ancho_barra=0.5, sep=0.2,
                     alto_normal=0.55, alto_torre=3.0, base_y=-1.7,
                     color=C_CREE):
    """Barras ESQUEMATICAS de un histograma (alturas arbitrarias, sin cifra
    por paso): una sola barra se dibuja como torre. No repite el histograma
    real de 181 pasos de la leccion 5.1; aqui solo importa la forma: casi
    todo parejo, un paso que se sale de la fila."""
    variacion = [0.62, 1.0, 0.74, 0.9, 0.66, 0.94, 0.78, 0.56, 0.86, 0.7]
    barras = VGroup()
    x0 = -(n - 1) * (ancho_barra + sep) / 2
    for i in range(n):
        h = alto_torre if i == indice_torre else alto_normal * variacion[i % len(variacion)]
        b = Rectangle(width=ancho_barra, height=h, stroke_color=color,
                     stroke_width=1.8, fill_color=color, fill_opacity=0.4)
        b.move_to(RIGHT * (x0 + i * (ancho_barra + sep)) + UP * (base_y + h / 2))
        barras.add(b)
    return barras
