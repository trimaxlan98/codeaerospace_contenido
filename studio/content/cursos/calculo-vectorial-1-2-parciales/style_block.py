# =====================================================================
# CO.DE Academy - "Cálculo vectorial · 1.2 Derivadas parciales: cortar el paisaje".
...
PAISAJE = paisaje_colinas                # el mismo paisaje que 1.1
Z_MAX = 2.2                              # tope de altura (para color_calor)

# El punto de corte de toda la leccion: en la ladera (no en la cima, donde
# el gradiente es 0 y no hay nada que cortar).
P0 = np.array([0.3, 0.1])
X0, Y0 = float(P0[0]), float(P0[1])
F0 = float(PAISAJE(P0))                  # altura en P0
DFDX = parcial(PAISAJE, P0, 0)           # pendiente del corte en x (clip 2)
DFDY = parcial(PAISAJE, P0, 1)           # pendiente del corte en y (clip 3)
GRAD0 = grad_num(PAISAJE, P0)            # (DFDX, DFDY) ensamblado

# clip 1: abanico de direcciones desde P0 con su subida medida (grad.u).
# E, NE, N, NO, O: NE es la mayor (anticipa el gradiente de 1.3); O baja.
ANGULOS_ABANICO = [0, 45, 90, 135, 180]


def _u_abanico(theta_grados):
    th = np.radians(theta_grados)
    return np.array([np.cos(th), np.sin(th)])


SUBIDAS_ABANICO = [float(np.dot(GRAD0, _u_abanico(a)))
                   for a in ANGULOS_ABANICO]

# clips 2/3: caja de la grafica 2D de cada corte (rango de f a lo largo
# del corte, con margen); ejes en x y en y comparten el mismo X0/Y0.
RANGO_CORTE = (-2.4, 2.4)
RANGO_F_CORTE_X = (0.3, 2.0)             # f(x, Y0) para x en RANGO_CORTE
RANGO_F_CORTE_Y = (0.0, 1.9)             # f(X0, y) para y en RANGO_CORTE


# clip 4: el plano tangente en P0 (la mejor aproximacion lineal) y el
# error de aproximarlo, medido cerca y lejos de P0 en la misma direccion.
def plano_tangente(p):
    p = np.asarray(p, dtype=float)
    d = p - P0
    return float(F0 + DFDX * d[0] + DFDY * d[1])


D_CERCA, D_LEJOS = 0.2, 1.6
P_CERCA = P0 + D_CERCA * np.array([1.0, 1.0])
P_LEJOS = P0 + D_LEJOS * np.array([1.0, 1.0])
ERROR_CERCA = abs(PAISAJE(P_CERCA) - plano_tangente(P_CERCA))
ERROR_LEJOS = abs(PAISAJE(P_LEJOS) - plano_tangente(P_LEJOS))
