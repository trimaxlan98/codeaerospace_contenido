# =====================================================================
# CO.DE Academy - "SDR · 7.3 GPS bajo el ruido".
...
PRN = 7
SNR = -20.0                                        # por muestra (param)
X = S.gps_captura(prn=PRN, snr_db=SNR)             # 10 ms, 2 muestras por chip
DOPS, REJ, (I_D, J_F), _ = S.gps_adquirir(X, prn=PRN)
PICO_DB = float(10 * np.log10(REJ.max() / REJ.mean()))     # ~13 dB
_R3 = S.gps_adquirir(X, prn=3)[1]
OTRO_DB = float(10 * np.log10(_R3.max() / _R3.mean()))     # ~4.5 dB
GANANCIA = S.ganancia_correlacion_db()             # 30.1 dB
CA = S.codigo_ca(PRN)
