# =====================================================================
# CO.DE Academy - "Procesamiento de señales · 2.1 LTI: el impulso lo dice todo".
...
# --- Numeros de la leccion --------------------------------------------
H_LTI = np.array([0.30, 0.25, 0.20, 0.15, 0.10])   # el sistema del clip
N_H = 10
H_IMPULSO = respuesta_impulso(H_LTI, [1.0], N_H)   # h[n] = los coeficientes
SUMA_H = suma_abs(H_IMPULSO)                       # 1.000
DELTA = impulso(N_H)

_RNG = np.random.default_rng(4)
X1 = np.round(_RNG.normal(0.0, 0.45, 20), 3)
X2 = np.round(_RNG.normal(0.0, 0.45, 20), 3)
A_LIN, B_LIN = 2.0, -1.5
Y1 = convolucion(X1, H_LTI)
Y2 = convolucion(X2, H_LTI)
X_MEZCLA = A_LIN * X1 + B_LIN * X2
Y_MEZCLA = convolucion(X_MEZCLA, H_LTI)
Y_APARTE = A_LIN * Y1 + B_LIN * Y2
ERR_LINEAL = float(np.max(np.abs(Y_MEZCLA - Y_APARTE)))    # 2.2e-16

K_RETARDO = 7
X_RETARDADA = np.concatenate([np.zeros(K_RETARDO), X1])
Y_RETARDADA = convolucion(X_RETARDADA, H_LTI)
DESPLAZAMIENTO = (int(np.argmax(np.abs(Y_RETARDADA)))
                  - int(np.argmax(np.abs(Y1))))            # 7

X_DESCOMP = np.array([0.9, -0.5, 0.7, 0.2, -0.8])
Y_DESCOMP = convolucion(X_DESCOMP, H_LTI)
