# =====================================================================
# CO.DE Academy - "Cálculo vectorial · 3.3 Campos conservativos: el camino no importa".
...
#
# El campo estrella de la leccion es F = grad(phi) con phi = x^2 y
# (phi_demo / F_demo de la libreria). A = (0, 0), B = (2, 1).
POT = phi_demo                           # el potencial phi = x^2 y
CAMPO = F_demo                           # F = grad(phi) = (2xy, x^2)

A_PT = np.array([0.0, 0.0])              # el punto de partida
B_PT = np.array([2.0, 1.0])              # el punto de llegada

# --- clip 1: tres caminos de A a B ------------------------------------
R_RECTA = camino_recta(A_PT, B_PT)
R_ARCO = camino_arco(A_PT, B_PT, 1.0)
R_ESC = camino_escalera(A_PT, B_PT, 4)
CAMINOS = (("recta", R_RECTA), ("arco", R_ARCO), ("escalera", R_ESC))
TRABAJOS = [integral_linea(CAMPO, r) for (_, r) in CAMINOS]   # 4.0, 4.0, 4.0


def sumas_camino(F, r, n=400, a=0.0, b=1.0):
    """(ts, sumas parciales de F.dr) a lo largo de r: el contador que corre
    se lee con np.interp sobre estas dos listas. Suma por puntos medios,
    robusta en las esquinas de la escalera (donde r' no existe)."""
    ts = np.linspace(a, b, int(n) + 1)
    pts = np.array([r(t) for t in ts], dtype=float)
    med = (pts[:-1] + pts[1:]) / 2.0
    dr = pts[1:] - pts[:-1]
    inc = np.array([float(np.dot(F(m), d)) for m, d in zip(med, dr)])
    return ts, np.concatenate([[0.0], np.cumsum(inc)])


SUMAS = [sumas_camino(CAMPO, r) for (_, r) in CAMINOS]

# --- clip 2: el circuito cerrado (ida por el arco, vuelta por la recta) -
def CIRCUITO(t):
    """A -> B por el arco (t en [0, 0.5]) y B -> A por la recta."""
    t = float(np.clip(t, 0.0, 1.0))
    if t <= 0.5:
        return R_ARCO(2.0 * t)
    return B_PT + (2.0 * t - 1.0) * (A_PT - B_PT)


W_CIRCUITO = circulacion(CAMPO, CIRCUITO)          # 0.0
TS_CIRC, S_CIRC = sumas_camino(CAMPO, CIRCUITO, n=800)

CIR_ROTOR = circulo((0.0, 0.0), 1.5)               # el circuito del rotor
W_ROTOR = circulacion(campo_rotor, CIR_ROTOR)      # 2*pi*1.5^2 = 14.14
TS_ROT, S_ROT = sumas_camino(campo_rotor, CIR_ROTOR, n=800)

# --- clip 3: el potencial y el teorema fundamental ---------------------
PHI_A = POT(A_PT)                        # 0.0
PHI_B = POT(B_PT)                        # 4.0
DELTA_PHI = PHI_B - PHI_A                # 4.0 = el trabajo medido
# phi = x^2 y crece rapido (en x en [-4,4] llega a +-60): el mapa se dibuja
# en un rango acotado y con niveles explicitos simetricos, o no se lee.
X0_MAPA, X1_MAPA, Y0_MAPA, Y1_MAPA = -3.0, 3.0, -2.5, 2.5
NIVELES_PHI = [-6.0, -4.0, -1.0, 0.0, 1.0, 4.0, 6.0]   # B cae en el 4.0
TS_POT, S_POT = SUMAS[0]                 # el altimetro de phi va por la recta

# --- clip 4: el test del rotacional y la gravedad ----------------------
P_TEST = ((0.6, 0.5), (1.6, -0.9), (-1.4, 1.4))    # donde va la ruedecita
ROT_GRAD = [rot_num(CAMPO, p) for p in P_TEST]     # 0.0, 0.0, 0.0
P_ROTOR = (0.0, 0.0)
ROT_ROTOR = rot_num(campo_rotor, P_ROTOR)          # 2.0
ERR_POT_GRAV = potencial_comprobado(campo_gravedad, phi_gravedad,
                                    -2.5, 2.5, -2.0, 2.0)   # ~0
ORBITA = circulo((0.0, 0.0), 1.6)                  # la orbita circular
W_ORBITA = circulacion(campo_gravedad, ORBITA)     # 0.0
TS_ORB, S_ORB = sumas_camino(campo_gravedad, ORBITA, n=800)

# --- escalas de dibujo -------------------------------------------------
MAG_REF = 10.0                           # tope de magnitud para color_calor
