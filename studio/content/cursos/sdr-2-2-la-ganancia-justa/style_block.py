# =====================================================================
# CO.DE Academy - "SDR · 2.2 La ganancia justa".
...
G, SINAD = S.snr_vs_ganancia()                    # 0..75 dB
G0, G1, GC, SMAX = S.punto_dulce(G, SINAD)          # 48, 60, 54, 30.3
SINAD_POCA = float(SINAD[0])                       # 6.3 dB (ganancia 0)
SINAD_MUCHA = float(SINAD[-1])                     # 8.2 dB (ganancia 75)
XG, WG = S.escenario_ganancia()                    # tono + ruido (SNR 30)
NIV_POCA = S.niveles_usados(10 ** (-60 / 20) * (XG + WG))   # 2 de 256
ESP_MUCHA, _ = S.espurio_max_dbc(66.0)             # -12.9 dBc
ESP_BIEN, _ = S.espurio_max_dbc(40.0)              # -41.0 dBc
ENV = S.desvanecimiento()                          # entrada (parametro)
SAL = S.agc(ENV)
VAR_ENT = float(ENV.max() - ENV.min())             # 30.7 dB
VAR_SAL = float(SAL[150:].max() - SAL[150:].min()) # 3.0 dB (tras enganchar)
