# =====================================================================
# CO.DE Academy - "Rendimiento SQL · 4.2 El tipo equivocado".
...
M = S.MEDIDO
EMAIL_NV = M["email_nvarchar"]                                  # 960 (motor)
EMAIL_V = M["email_varchar"]                                    # 3 (motor)
RAZON_EMAIL = S.razon(EMAIL_NV, EMAIL_V)                        # 320.0 (calculo)
FILAS_CLI = M["filas_clientes"]                                 # 200,000 (motor)
...
def fila_bytes(n, ancho_char, alto=0.5, color=None, buff=0.03):
    """Una fila de `n` celdas SIN texto (cada celda = un caracter de la
    cadena que se rotula encima): el ANCHO de la celda es lo que cambia
    entre VARCHAR (1 byte) y NVARCHAR (2 bytes) para la MISMA cantidad de
    caracteres. No lleva cifra: es una comparacion de tamano relativo."""
    color = C_INDICE if color is None else color
    celdas = VGroup(*[
        Rectangle(width=ancho_char, height=alto, stroke_color=color,
                  stroke_width=1.6, fill_color=color, fill_opacity=0.22)
        for _ in range(n)])
    celdas.arrange(RIGHT, buff=buff)
    return celdas


def icono_aviso(lado=0.5, color=None):
    """Triangulo de advertencia con un '!' dentro: SIEMPRE un icono, nunca
    una cifra (regla de la leccion: el ambar de aviso solo vale si se lee
    inequivocamente como advertencia)."""
    color = C_MOTOR if color is None else color
    tri = Triangle(color=color, fill_color=color, fill_opacity=0.12,
                   stroke_width=3.2)
    tri.scale(lado)
    palo = Line(tri.get_center() + UP * lado * 0.32,
               tri.get_center() + DOWN * lado * 0.06,
               stroke_color=color, stroke_width=3.0)
    punto = Dot(tri.get_center() + DOWN * lado * 0.28, radius=lado * 0.045,
               color=color)
    g = VGroup(tri, palo, punto)
    g.tri = tri
    return g


def entrada_indice(texto, ancho=3.7, alto=0.46, color=None):
    """Una hoja del indice ordenado por email: una caja angosta con el
    correo dentro (Space Mono, ASCII)."""
    color = C_INDICE if color is None else color
    caja = RoundedRectangle(width=ancho, height=alto, corner_radius=0.06,
                            stroke_color=color, stroke_width=2.0,
                            fill_color=S.FONDO_PANEL, fill_opacity=0.9)
    t = Text(texto, font=FUENTE_HUD, font_size=18, color=color)
    if t.width > ancho - 0.2:
        t.scale_to_fit_width(ancho - 0.2)
    t.move_to(caja)
    g = VGroup(caja, t)
...
def lista_indice_email(correos, ancho=3.7, alto=0.46, buff=0.1, color=None):
    """La columna ordenada por email (una entrada por correo, de arriba
    hacia abajo en orden alfabetico: `correos` ya debe venir ordenada)."""
    filas = VGroup(*[entrada_indice(c, ancho=ancho, alto=alto, color=color)
                     for c in correos])
    filas.arrange(DOWN, buff=buff)
    return filas
