# =====================================================================
# CO.DE Academy - "SDR · 2.1 El piso de ruido".
...
KTB_HZ = S.ktb_dbm(1)                              # -174.0 dBm/Hz
KTB_2K = S.ktb_dbm(2e3)                            # -141.0
KTB_200K = S.ktb_dbm(200e3)                        # -121.0
KTB_2M4 = S.ktb_dbm(2.4e6)                         # -110.2
NF_ANT = S.friis([S.LNA, S.CABLE, S.RECEPTOR])     # 1.04 dB
NF_ESC = S.friis([S.CABLE, S.LNA, S.RECEPTOR])     # 3.91 dB
NF_SIN = S.friis([S.CABLE, S.RECEPTOR])            # 9.0 dB
SNR_FM, SNR_BAL = 12.0, 6.0                        # SNR minimas (datos)
SENS_FM = S.sensibilidad_dbm(200e3, NF_ANT, SNR_FM)    # -107.9 dBm
SENS_BAL = S.sensibilidad_dbm(2e3, NF_ANT, SNR_BAL)    # -133.9 dBm
P_DEBIL = -125.0                                   # senal de prueba (dato)
SNR_ANT = S.snr_medida(P_DEBIL, 2e3, NF_ANT)       # 14.9 dB
SNR_ESC = S.snr_medida(P_DEBIL, 2e3, NF_ESC)       # 12.0 dB
