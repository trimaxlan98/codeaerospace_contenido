#!/usr/bin/env python3
"""Reparte el guion de voz del curso 36 en los ficheros que `guiones.py
--solo-audio` sintetiza.

El VPS no tiene guionista (MS_TTS_GUIONISTA=ninguno): la narracion se
escribe a mano en `docs/plan_contenido/curso-36-guion-de-voz.md`, un bloque
...
`guiones/<slugify(nombre del proyecto)>/`:
    ...
    python3 studio/tools/voz_tesis6g.py [--salida guiones] [--comprobar]
...
GUION = RAIZ / "docs/plan_contenido/curso-36-guion-de-voz.md"
...
def main() -> int:
    ...
    g = leer_guion()
    problemas = 0
    for d in sorted(CURSOS.glob("tesis6g-*")):
        m = re.match(r"tesis6g-(\d+)-(\d+)-", d.name)
